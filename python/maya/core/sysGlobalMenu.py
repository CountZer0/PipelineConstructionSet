...
#		self.p4Active = eval(self.userXMLCore.get('P4Active'))
	...
	def injectPaths(self):
		...
		if search('x64', mVer):
			mVer = mVer.split(' ')[0]
			putEnv("MAYA_PLUG_IN_PATH", [getEnv('MAYA_PLUG_IN_PATH'), '%s/plugins/%s/win64' % (self.PCSmenuPath, mVer), '%s/plugins/%s/win64/others' % (self.PCSmenuPath, mVer)])
			sys.path.append('%s/python/common/pyQt/PyQt4/2012/win64' % self.PCStoolsLocation)
		else:
			putEnv("MAYA_PLUG_IN_PATH", [getEnv('MAYA_PLUG_IN_PATH'), '%s/plugins/%s/win32' % (self.PCSmenuPath, mVer), '%s/plugins/%s/win32/others' % (self.PCSmenuPath, mVer)])
			sys.path.append('%s/python/common/pyQt/PyQt4/2012/win32' % self.PCStoolsLocation)
		# Add extraScripts/include folder	
		putEnv("MAYA_SCRIPT_PATH", [getEnv('MAYA_SCRIPT_PATH'), '%s/extraScripts/include/%s' % (self.PCSmenuPath, mVer)])
	...
	def PCSMenu_xPrefsInit(self):
		""" Initialize xPrefs """
		if os.path.exists('%s%s' % (internalVar(userScriptDir=True), 'mayaToolsMenuPrefs_xPrefs.mel')): #@UndefinedVariable
			mel.eval('source "%s%s"' % (internalVar(userScriptDir=True), 'mayaToolsMenuPrefs_xPrefs.mel')) #@UndefinedVariable
			scriptJob(event=('SceneOpened', "mel.mayaToolsMenuPrefs_xPrefs()"))
			scriptJob(event=('NewSceneOpened', "mel.mayaToolsMenuPrefs_xPrefs()"))

	def PCSMenu_loggerMenuInit(self):	
		""" Add logging menu """
	...
	def PCSMenu_localFileCopy(self):
		...
		if usrSetupFile.exists():
			targetPath = '%s/data/%s/%s_networkCopy%s' % (self.PCSlogLocation, self.userName, usrSetupFile.namebase, usrSetupFile.ext)
			copyfile(usrSetupFile, targetPath)

		# Maya.env
		if mayaEnvFile.exists():
			targetPath = '%s/data/%s/%s_networkCopy%s' % (self.PCSlogLocation, self.userName, mayaEnvFile.namebase, mayaEnvFile.ext)
	...
	def PCSMenu_build(self):
		...
		if not cmds.menuItem(self.menuName, q=1, ex=1):
			self.PCSMenu = Menu(self.menuName, parent=melGlobals['gMainWindow'], allowOptionBoxes=1, tearOff=1, label=self.PCStoolName)
		else:
		...
		for root, unused, unused in os.walk(os.path.abspath(self.PCSmenuStart)):
			if not pcsPath.Path(root).makePretty(0, 0) == self.PCSmenuStart:
				...
				for fname in os.listdir(root):
					...
					if pcsPath.Path(fullPath).isfile() and (fnmatch.fnmatch(fname, '*.mel') or fnmatch.fnmatch(fname, '*.div')):
						mFiles.append(fname) 
...
#							pPath = parser_PCS.ParsePCS().userGet('PCSprojectToolPath')
							...
							if os.path.exists('%s/teamSave.mel' % lookPath):
								...
								mel.eval('source "%s/maya/mel/overload/saveOverload_%s.mel"' % (pPath, about(version=1).split(' ')[0]))
								mel.eval('source "%s/teamSave.mel"' % lookPath)
							else:
								# Source the original autodesk procs
								mel.eval('source "%s/scripts/others/pv_performAction.mel"' % mel.getenv('MAYA_LOCATION'))
