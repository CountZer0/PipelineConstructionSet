...
class FileReference(object):
    ...
    def exportAnim(self, exportPath, **kwargs):
        ...
        Flags:
          ...
          - referenceNode:
              When queried, this flag returns the name of the reference node associated with the file. If the file is not a reference
              or does not have a reference node, an empty string is returned. In query mode, this flag can accept a value.
          - type:
    ...
    def load(self, newFile=None, **kwargs):
        ...
        Flags:
          - loadNoReferences:
              ...
              references will be loaded. When used with -import, -reference or -loadReference flags, will load the top-most reference
              only.
          - loadReferenceDepth:
              Used to specify which references should be loaded. Valid types are "all", "none" and "topOnly", which will load all
              references, no references and top-level references only, respectively. Must be used with the -open, -import, -reference
              or -loadReference flags. When "none" is used with -loadReference, only path validation is performed. This can be used to
          ...
          - returnNewNodes:
              Used to control the return value in open, import, loadReference, and reference operations. It will force file command to
              return a list of new nodes added to the current scene.
...
def setAttrMapping(*args, **kwargs):
    ...
    Flags:
      ...
      - relative : r                   (bool)          [create]
          Make the mapping relative.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a
          tuple or a list.
...
def audioTrack(*args, **kwargs):
    ...
    also be used to modify the track state, for example, to lock or mute a track. In query mode, return type is based on
    queried flag.
    ...
    Flags:
      ...
      - numTracks : nt                 (int)           [query]
          To query the number of audio tracksFlag can appear in Create mode of commandFlag can have multiple arguments, passed
...
def shotTrack(*args, **kwargs):
    ...
    used to modify the track state, for example, to lock or mute a track. In query mode, return type is based on queried
    flag.
    ...
    Flags:
      ...
      - numTracks : nt                 (int)           [query]
          To query the number of tracksFlag can appear in Create mode of commandFlag can have multiple arguments, passed either as
          a tuple or a list.
...
def attachDeviceAttr(*args, **kwargs):
    ...
    Flags:
      ...
      - cameraRotate : cr              (bool)          [create]
          This flag attaches the device/axis to the current cameras rotation controls.Flag can appear in Create mode of
      ...
      - selection : sl                 (bool)          [create]
          ...
          changing the selection will change the attachments.
...
def moduleInfo(*args, **kwargs):
    ...
    Flags:
      ...
      - moduleName : mn                (unicode)       [create]
          ...
          flags.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def findType(*args, **kwargs):
    """
    The findTypecommand is used to search upstream through a dependency subgraph on a certain node to find all nodes of the
    ...
    Flags:
      ...
      - type : t                       (unicode)       [create]
          Type of node to look for (e.g. "transform"). This flag is mandatory.Flag can appear in Create mode of commandFlag can
...
def aaf2fcp(*args, **kwargs):
    ...
    Flags:
      ...
      - terminate : t                  (int)           [create]
          Complete the taskFlag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or
          a list.
...
def dbmessage(*args, **kwargs):
    ...
    Flags:
      - file : f                       (unicode)       [create]
          ...
          window of Developer Studio. Default value is stdout.Flag can appear in Create mode of commandFlag can have multiple
...
def dgtimer(*args, **kwargs):
    ...
    various operations we measure are called "metrics" and the types of timers are called "timer types". The various metrics
    are always measured when timing is on, but are only queried when specified via the -show and -hide flags. The metrics
    currently supported are listed in detail under the -show flag below. For each metric we support a standard set of timer
    types. There are three of these: "self" for self time (the time specific to the node and not its children), "inclusive"
    (time including children of the node), and "count" (number of operations of the given metric on the node). The timing
    mechanism which is used by dgtimeris built into the DG itself, thus ALL depend nodes can be timed and there is no need
    for programmers writing plug-ins using the OpenMaya API to add any special code in order for their nodes to be timed --
    its all handled transparently. The dgtimercommand allows node timers to be turned on, off, reset to zero, and have their
    current value displayed, and these operations can be performed globally on all nodes or on a specific set of nodes
    defined by name, type or parentage. Note that all timer measurements are computed in "real time" (the same time
    measurement you get from a wristwatch) as opposed to "CPU time" (which only measures time when the processor is
    executing your code). All times are displayed in seconds. Use the -query flag to display the current timer values on a
    node,use -on to turn on timing,use -off to turn off timing,and -reset to reset timers to zero. To display the values
    measured during timing, there are two approaches. The first method is to use the -query flag can be used to report the
    information which has been measured. The second method is to use the query methods available on the OpenMaya class
    MFnDependencyNode (see the OpenMaya documentation for details). What follows is a description of what is generated via
    -query. The output is broken out into several sections and these are described as follows: SECTION 1:Section 1 of the
    dgtimer output contains global information. This section can be disabled via the -hoHeader flag. These values are reset
    whenever a global timer reset occurs (i.e. dgtimer -reset;is specified). The global values which are reported are: Total
    real time:the total wall-clock time since the last global timer reset. This is the actual time which has been spent as
    you might measure it measure it with your watch. On a multi-processing system, this value will always remain true to to
    real time (unlike userand systime).Total user time:the total time the CPU(s) spent processing Maya not including any
    system time since the last global timer reset.Total sys time:the total time the CPU(s) spent in operating system calls
    on behalf of Maya since the last global timer reset.Summary of each metric for all nodes:a summary of self and count for
    each metric that we measure:Real time in callbacksreports the self time and count for the "callback" metric.Real time in
    computereports the self time and count for the "compute" metric.Real time in dirty propagationreports the self time and
    count for the "dirty" metric.Real time in drawingreports the self time and count for the "draw" metric.Real time
    fetching data from plugsreports the self time and count for the "fetch" metric.Breakdown of select metrics in greater
    detail:a reporting of certain combinations of metrics that we measure:Real time in compute invoked from callbackreports
    the self time spent in compute when invoked either directly or indirectly by a callback.Real time in compute not invoked
    from callbackreports the self time spent in compute not invoked either directly or indirectly by a callback.SECTION
    2:Section 2 of the dgtimer -query output contains per-node information. There is a header which describes the meaning of
    each column, followed by the actual per-node data, and this is ultimately followed by a footer which summarises the
    totals per column. Note that the data contained in the footer is the global total for each metric and will include any
    nodes that have been deleted since the last reset, so the value in the footer MAY exceed what you get when you total the
    individual values in the column. To prevent the header and footer from appearing, use the -noHeader flag to just display
    the per-node data. The columns which are displayed are as follows: Rank:The order of this node in the sorted list of all
    nodes, where the list is sorted by -sortMetric and -sortType flag values (if these are omitted the default is to sort by
    self compute time).ON:Tells you if the timer for that node is currently on or off. (With dgtimer, you have the ability
    to turn timing on and off on a per-node basis).Per-metric information:various columns are reported for each metric. The
    ...
    operation (thus if the metric is "DRAW", then this will be time spent drawing the node).Inclusive:The amount of real
    time (i.e. elapsed time as you might measure it with a stopwatch) spent performing the operation including any child
    ...
    the metric is "DRAW", then the number of draw operations on the node will be reported).Sort informationif a column is
    the one being used to sort all the per-node dgtimer information, then that column is followed by a Percentand
    Cumulativecolumn which describe a running total through the listing. Note that "-sortType none" prevents these two
    columns from appearing and implicitely sorts on "self" time.After the per-metric columns, the node name and type are
    ...
    (number of times the callbackId was invoked).API lists "Y" if the callbackId was defined through the OpenMaya API, and
    "N" if the callbackId was defined internally within Maya.Node lists the name of the node this callbackId was associated
    with. If the callbackId was associated with more than one node, the string "\*multiple\*" is printed. If there was no
    node associated with the callbackId (or its a callback type in which the node is hard to deduce), the entry is
    blank.After the callbackId entries are listed, a dashed line is printed followed by a single line listing the self,
    inclusive and count values for the callback. Note that the percent is relative to the global callback time.At the bottom
    of SECTION 3.1 is the per-column total. The values printed match the summation at the bottom of the listing in section
    2. Note that the values from SECTION 3.1 include any nodes that have been deleted since the last reset. The thresholding
    parameters (-threshold, -rangeLower, -rangeUpper and -maxDisplay) are honoured when generating the listing. The sorting
    of the rows and display of the Percent and Cumulative columns obeys the -sortType flag. As the listing can be long, zero
    entries are not displayed. The second part is SECTION 3.2 which lists the data per callbackId. As noted earlier, the
    callbackId is an identifying number which is unique to each client that is registered to a callback and can be deduced
    by the user, such as through the OpenMaya API. The entries in SECTION 3.2 appear as follows: CallbackId the numeric
    identifier for the callback. You can cross reference by finding the same callbackId value listed in SECTIONs 3.1 and
    3.3.For each callbackId, the data is broken down per-callback:Callback the name of the callback, e.g.
    "attributeChangedMsg".Percent, Cumulative, Inclusive, Count, API and Node entries as described in SECTION 3.1.After the
    callback entries are listed for the callbackId, a dashed followed by a summary line is printed. The summary line lists
    the self, inclusive and count values for the callback. Note that the percent is relative to the global callback time.The
    third part is SECTION 3.3 which lists data per-callback per-node. The nodes are sorted based on the -sortType flag, and
    for each node, the callbacks are listed, also sorted based on the -sortType flag. As this listing can be long, zero
    entries are not displayed. An important note for SECTION 3.3 is that only nodes which still exist are displayed. If a
    node has been deleted, no infromation is listed.
    ...
    Flags:
      ...
      - hierarchy : h                  (bool)          [create,query]
          Used to specify that a hierarchy of the dependency graph be affected, thus "-reset -hierarchy -name ball" will reset the
          timers on the node named "ball" and all of its descendents in the dependency graph.
      ...
      - name : n                       (unicode)       [create,query]
          ...
          omitted). Note that one can force output to the script editor window via the "-outputFile MEL" option to make it easy to
      ...
      - outputFile : o                 (unicode)       [query]
          ...
          in the Maya Script Editor (only supported with -query).The "stdout" setting is the default behaviour. This flag can be
          used with the -query flag as well as the -trace flag. When used with the -trace flag, any tracing output will be
          displayed on the destination specified by the -outputFile (or stdout if -outputFile is omitted). Tracing operations will
          continue to output to the destination until you specify the -trace and -outputFile flags again. When used with the
          -query flag, timing output will be displayed to the destination specified by the -outputFile (or "stdout" if -outputFile
          is omitted). Here are some examples of how to use the -query, -trace and -outputFile flags: Example: output the timing
          information to a single file on disk:dgtimer -on;                                       // Turn on timing create some
          animated scene content; play -wait;                                        // Play the scene through dgtimer -query
          -outputFile "/tmp/timing.txt"       // Output node timing information to a file on disk Example: output the tracing
          information to a single file on disk:dgtimer -on;                                       // Turn on timing create some
          animated scene content; dgtimer -trace on -outputFile "/tmp/trace.txt"     // Turn on tracing and output the results to
          file play -wait;                                        // Play the scene through; trace info goes to /tmp/trace.txt
          dgtimer -query;                                    // But the timing info goes to the terminal window play -wait;
          // Play the scene again, trace info still goes to /tmp/trace.txt Example: two runs, outputting the trace information and
          timing information to separate files:dgtimer -on;                                       // Turn on timing create some
          animated scene content; dgtimer -trace on -outputFile "/tmp/trace1.txt"    // Turn on tracing and output the results to
          file play -wait;                                        // Play the scene through dgtimer -query -outputFile
          "/tmp/query1.txt"       // Output node timing information to another file dgtimer -reset; dgtimer -trace on -outputFile
          "/tmp/trace2.txt"    // Output tracing results to different file play -wait;                                        //
          Play the scene through dgtimer -query -outputFile "/tmp/query2.txt"       // Output node timing information to another
          file Tips and tricks:Outputting the timing results to the script editor makes it easy to use the results in MEL e.g.
          string $timing[] = `dgtimer -query -outputFile MEL`.It is important to note that the -outputFile you specify with -trace
          is totally independent from the one you specify with -query.If the file you specify already exists, Maya will empty the
      ...
      - overhead : oh                  (bool)          [create,query]
          ...
          default this option is turned off. To enable it, specify "dgtimer -overhead true" prior to starting timing. When
          querying timing, the overhead is reported in SECTION 1.2 of the dgtimer output and is not factored out of each
      ...
      - rangeUpper : rgu               (float)         [create]
          ...
          above the lower range limit are considered.Flag can appear in Create mode of commandFlag can have multiple arguments,
      ...
      - returnType : rt                (unicode)       [query]
          ...
          output. The flag accepts the values "total", to just display the column total, or "all" to display all entries
          individually. For example, if you want to get the total of draw self time without any other output simply specify the
          following:dgtimer -returnType total -sortMetric draw -sortType self -threshold 100 -noHeader -query;// Result: 7718.01
          // To instead get each individual entry, change the above query to:dgtimer -returnType all -sortMetric draw -sortType
          self -threshold 100 -noHeader -query;// Result: 6576.01 21.91 11.17 1108.92 // To get the inclusive dirty time for a
          specific node, use -name as well as -returnType all:dgtimer -name "virginia" -returnType all -sortMetric dirty -sortType
          inclusive -threshold 100 -noHeader -query;Note: to get the total number of nodes, use "-sortType none -returnType
          total". To get the on/off status for each node, use "-sortType none -returnType all".
      ...
      - show : sh                      (unicode)       [create,query]
          ...
          methods), "dirty" (to display time spent propagating dirtiness on behalf of the node), "draw" (to display time spent
          drawing the node), "compcb" (to display time spent during callback processing on node due to compute), and "compncb" (to
          display time spent during callback processing on node NOT due to compute). The -show flag can be used multiple times,
          but cannot be specified with -hide. By default, if neither -show, -hide, or -sort are given, the effective display mode
      ...
      - sortMetric : sm                (unicode)       [create,query]
          ...
          generated, for example "draw" time. Note that the -sortType flag can also be specified to define which timer is sorted
          on: for example "dgtimer -sortMetric draw -sortType count -query" will sort the output by the number of times each node
          was drawn. Both -sortMetric and -sortType are optional and you can specify one without the other. The -sortMetric flag
          can only be specified at most once. The flag takes the following arguments: "callback" (to sort on time spent during any
          callback processing on the node), "compute" (to sort on the time spent in the node's compute methods), "dirty" (to sort
          on the time spent propagating dirtiness on behalf of the node), "draw" (to sort on time spent drawing the node), "fetch"
          (to sort on time spent copying data from the datablock), The default, if -sortMetric is omitted, is to sort on the first
          displayed column. Note that the sortMetric is independent of which columns are displayed via -show and -hide. Sort on a
          hidden column is allowed. The column selected by -sortMetric and -sortType specifies which total is returned by the
          dgtimer command on the MEL command line. This flag is also used with -updateHeatMap to specify which metric to build the
          heat map for.
      ...
      - sortType : st                  (unicode)       [create,query]
          ...
          generated, for example "self" time. Note that the -sortMetric flag can also be specified to define which metric is
          sorted on: for example "dgtimer -sortMetric draw -sortType count -query" will sort the output by the number of times
          each node was drawn. Both -sortMetric and -sortType are optional and you can specify one without the other. The
          -sortType flag can be specified at most once. The flag takes the following arguments: "self" (to sort on self time,
          which is the time specific to the node and not its children), "inclusive" (to sort on the time including children of the
          node), "count" (to sort on the number of times the node was invoked). and "none" (to sort on self time, but do not
          display the Percent and Cumulative columns in the per-node display, as well as cause the total number of nodes in Maya
          to be returned on the command line). The default, if -sortType is omitted, is to sort on self time. The column selected
          by -sortMetric and -sortType specifies which total is returned by the dgtimer command on the MEL command line. The
          global total as displayed in SECTION 1 of the listing is returned. The special case of "-sortType none" causes the
          number of nodes in Maya to instead be returned. This flag is also used with -updateHeatMap to specify which metric to
      ...
      - threshold : th                 (float)         [query]
          ...
          used for sorting. For example, if our sort key is self compute time (i.e. -sortMetric is "compute" and -sortType is
          "self") and the threshold parameter is 20.0, then only nodes with a compute self-time of 20.0 or higher will be
          displayed. (Note that -threshold uses absolute time. There are the similar -rangeUpper and -rangeLower parameters which
          specify a range using percentage).
      ...
      - trace : tr                     (bool)          [create]
          ...
          particleShape1Deformed.lastPositionThe above is an example of the output when -trace is true that marks the start of an
          operation. For specific details on each field: the "dgtimer:begin:" string is an identifying marker to flag that this is
          a begin operation record. The second argument, "compute" in our example, is the operation metric. You can view the
          output of each given metric via "dgtimer -q" by specifying the -show flag. The integer which follows (3 in this case) is
          the depth in the operation stack, and the third argument is the name of the node (particleShape1Deformed). The fourth
          argument is specific to the metric. For "compute", it gives the name of the plug being computed. For "callback", its the
          internal Maya name of the callback. For "dirty", its the name of the plug that dirtiness is being propagated
          from.dgtimer:end: compute 3 particleShape1Deformed 0.000305685 0.000305685The above is the end operation record. The
          "compute", "3" and "particleShapeDeformed" arguments were described in the "dgtimer:begin" overview earlier. The two
          floating-point arguments are self time and inclusive time for the operation measured in seconds. The inclusive measure
          lists the total time since the matching "dgtimer:begin:" entry for this operation, while the self measure lists the
          inclusive time minus any time consumed by child operations which may have occurred during execution of the current
          operation. As noted elsewhere in this document, these two times are "wall clock times", measuring elapsed time including
          any time in which Maya was idle or performing system calls. Since dgtimer can measure some non-node qualities in Maya,
          such as global message callbacks, a "-" is displayed where the node name would ordinarily be displayed. The "-" means
          "not applicable".
      ...
      - uniqueName : un                (bool)          [create,query]
          Used to specify that the DAG nodes listed in the output should be listed by their unique names. The full DAG path to the
          object will be printed out instead of just the node name.
...
def dirmap(*args, **kwargs):
    ...
    mappings and enabled state are not preserved between Maya sessions. This command requires one "main" flag that specifies
    ...
    Flags:
      - convertDirectory : cd          (unicode)       [create]
          ...
          trailing '/'.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
...
def sceneEditor(*args, **kwargs):
    ...
    Flags:
      ...
      - withoutCopyNumber : wcn        (bool)          [query]
          ...
          the end. If this flag is not present, the file name returned may have a copy number appended to the end.Flag can appear
...
def namespace(*args, **kwargs):
    ...
    -set ":";'. Whenever an object is created, it is added by default to the current namespace. When creating a new
    namespace using a qualified name, any intervening namespaces which do not yet exist will be automatically created. For
    example, if the name of the new namespace is specified as "A:B" and the current namespace already has a child namespace
    named "A" then a new child namespace named "B" will be created under "A". But if the current namespace does not yet have
    a child named "A" then it will be created automatically. This applies regardless of the number of levels in the provided
    name (e.g. "A:B:C:D"). The 'p/parent' flag can be used to explicitly specify the parent namespace under which the new
    one should be created, rather than just defaulting to the current namespace. If the name given for the new namespace is
    absolute (i.e. it begins with a colon, as in ":A:B") then both the current namespace and the 'parent' flag will be
    ignored and the new namespace will be created under the root namespace. The relativeNamespace flag can be used to change
    the way node names are displayed in the UI and returned by the 'ls' command. Here are some specific details on how the
    return from the 'ls' command works in relativeNamespace mode: List all mesh objects in the scene:ls -type "mesh";The
    above command lists all mesh objects in the root and any child namespaces. In relative name lookup mode, all names will
    be displayed relative to the current namespace. When not in relative name lookup mode (the default behaviour in Maya),
    results are printed relative to the root namespace.Using a "\*" wildcard:namespace -set myNS;ls -type "mesh "\*";In
    relative name lookup mode, the "\*" will match to the current namespace and thus the ls command will list only those
    meshes defined within the current namespace (i.e. myNs). If relative name lookup is off (the default behaviour in Maya),
    names are root-relative and thus "\*" matches the root namespace, with the net result being that only thoses meshes
    defined in the root namespace will be listed.You can force the root namespace to be listed when in relative name lookup
    mode by specifying ":\*" as your search pattern (i.e. ls -type mesh ":\*" which lists those meshes defined in the root
    namespace only). Note that you can also use ":\*" when relative name lookup mode is off to match the root if you want a
    consistent way to list the root.Listing child namespace contents:ls -type mesh "\*:\*";For an example to list all meshes
    in immediate child namespaces, use "\*:\*". In relative name lookup mode "\*:\*" lists those meshes in immediate child
    namespaces of the current namespaces. When not in relative name lookup mode, "\*:\*" lists meshes in namespaces one
    level below the root.Recursive listing of namespace contents:Example: ls -type mesh -recurse on "\*"The 'recurse' flag
    is provided on the "ls" command to recursively traverse any child namespaces. In relative name lookup mode, the above
    example command will list all meshes in the current and any child namespaces of current. When not in relative name
    lookup mode, the above example command works from the root downwards and is thus equivalent to "ls -type mesh".
    ...
    Flags:
      - absoluteName : an              (bool)          [create,query]
          ...
          namespaces. For example ":ns:ball" is an absolute namespace name while "ns:ball" is not.
    
      - addNamespace : add             (unicode)       [create]
          Create a new namespace with the given name. Both qualified names ("A:B") and unqualified names ("A") are acceptable. If
          any of the higher-level namespaces in a qualified name do not yet exist, they will be created.
      ...
      - collapseAncestors : ch         (unicode)       [create]
          ...
          or other nested namespacesFlag can appear in Create mode of commandFlag can have multiple arguments, passed either as a
          tuple or a list.
      ...
      - relativeNames : rel            (bool)          [create,query]
          ...
          leave relativeNames turned on, if you try to toggle the value during file I/O you may notice that the value stays "on"
          because Maya has already temporarily enabled it internally.When relativeNames are enabled, the returns provided by the
          'ls' command will be relative to the current namespace. See the main description of this command for more details.
      ...
      - rename : ren                   (unicode, unicode) [create]
          ...
          if the second namespace name already exists.
...
def cacheFileCombine(*args, **kwargs):
    ...
    Flags:
      ...
      - channelName : cnm              (unicode)       [edit]
          ...
          the first channel in the file is used.Flag can appear in Create mode of commandFlag can have multiple arguments, passed
          either as a tuple or a list.
      ...
      - objectIndex : oi               (int)           [query,edit]
          ...
          returns the index related to the object specified with the object flag.
...
def loadModule(*args, **kwargs):
    ...
    Flags:
      - allModules : a                 (bool)          [create]
          Load all new modules not yet loaded in Maya. New modules are the one returned by the -scan option.Flag can appear in
      ...
      - scan : sc                      (bool)          [create]
          ...
          any of these newly found modules, not change the Maya state.
...
def unloadPlugin(*args, **kwargs):
    ...
    Flags:
      ...
      - removeCallback : rc            (script)        [create]
          Remove a procedure which was previously added with -addCallback.Flag can appear in Create mode of commandFlag can have
...
def createReference(filepath, **kwargs):
    ...
    Flags:
      ...
      - loadReferenceDepth:
          ...
          unloaded state that they were in when the file was saved.
      - defaultNamespace:
          Use the default name space for import and referencing. This is an advanced option. If set, then on import or reference,
          Maya will attempt to place all nodes from the imported or referenced file directly into the root (default) name space,
          without invoking any name clash resolution algorithms. If the names of any of the new objects already exist in the root
          namespace, then errors will result. The user of this flag is responsible for creating a name clash resolution mechanism
          outside of Maya to avoid such errors. Note:This flag is intended only for use with custom file translators written
          through the API. Use at your own risk.
      - deferReference:
      ...
      - namespace:
          ...
          Namespaces" option, and must be loaded. Non-referenced nodes contained in the existing namespace will also be moved to
          the new namespace. The new namespace will be created by this command and can not already exist. The old namespace will
          be removed.
      - referenceNode:
          When queried, this flag returns the name of the reference node associated with the file. If the file is not a reference
          or does not have a reference node, an empty string is returned. In query mode, this flag can accept a value.
      ...
      - swapNamespace:
          ...
          given namespace to an alternate specified namespace. This namespace "swap" will occur as the file is referenced in. It
      ...
      - sharedReferenceFile:
          ...
          nodes. New copies of those nodes will not be created if a copy already exists. Instead, the shared node will be merged
          with the existing node. The specifics of what happens when two nodes are merged depends on the node type. This flag
      ...
      - sharedNodes:
          ...
          and shading networks. Two shading networks will only be merged if they are identical: the network of nodes feeding into
          the shading group must be arranged identically with equivalent nodes have the same name and node type. Additionally if a
          network is animated or contains a DAG object or expression it will not be mergeable. This flag cannot be used in
...
def sceneUIReplacement(*args, **kwargs):
    ...
    Flags:
      ...
      - deleteRemaining : dr           (bool)          [create]
          Delete any UI that is scene dependent and has not been referenced by this command since the last update.Flag can appear
          in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
      ...
      - update : u                     (unicode)       [create]
          Updates the state of the command to reflect the current state of the application. The string argument is the name of the
...
def showHelp(*args, **kwargs):
    ...
    Flags:
      ...
      - helpTable : ht                 (bool)          [create,query]
          ...
          directory.If this flag does not accept an argument if it is queried.The default value is "helpTable".Flag can appear in
          Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def cacheFileMerge(*args, **kwargs):
    ...
    Flags:
      - endTime : et                   (time)          [create]
          Specifies the end frame of the merge range. If not specified, will figure out range from times of caches being
          merged.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def dgeval(*args, **kwargs):
    """
    The dgevalcommand is used to force a dependency graph evaluate of a node or plug. Used for debugging to find propagation
    problems.
    ...
    Flags:
      ...
      - verbose : v                    (bool)          [create]
          If this flag is used then the results of the evaluation(s) is/are printed on stdout. -src If specified then evaluate
          node outputs as well as inputs when evaluating an entire node. objectListNormally the selection list is used to
          determine which objects to evaluate, but you can add to the selection list by specifying which objects you want on the
          command line.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
...
def referenceEdit(*args, **kwargs):
    ...
    applied. By default this command only works on failed edits (this can be adjusted using the "-failedEdits" and
    "-successfulEdits" flags). Specifying a reference node as the command target is equivalent to specifying every node in
    the target reference file as a target. In this situation the results may differ depending on whether the target
    reference is loaded or unloaded. When it is unloaded edits which affect both a node in the target reference and a node
    in one of its decendant references may be missed (e.g. they may not be removed). Edits which only affect nodes in the
    target reference or one of its ancestral references, however, should be removed as expected. This is because when a
    reference is unloaded Maya no longer retains detailed information about which nodes belong to it. NOTE: When specifying
    ...
    Flags:
      ...
      - changeEditTarget : cet         (unicode, unicode) [create]
          ...
          target to change it to. The target can either be a node name ("node"), a node and attribute name ("node.attr"), or just
          an attribute name (".attr"). If an edit currently affects the old target, it will be changed to affect the new target.
          You should use 'referenceQuery' to determine the format of the edit targets. As an example most edits store the long
          name of the attribute (e.g. "translateX"), so when specifying the old target, a long name must also be used. If the
          short name is specified (e.g. "tx"), chances are the edit won't be retargeted.
      ...
      - editCommand : ec               (unicode)       [create,query]
          ...
          flag is not specified all edit types will be included. This flag requires a string parameter. Valid values are:
          "addAttr", "connectAttr", "deleteAttr", "disconnectAttr", "parent", "setAttr", "lock" and "unlock". In some contexts,
          this flag may be specified more than once to specify multiple edit types to consider.
      ...
      - onReferenceNode : orn          (unicode)       [create,query]
          ...
          be considered. This flag only supports multiple uses when specified with the "exportEdits" command.Flag can appear in
          Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def clearCache(*args, **kwargs):
    ...
    command goes in to all of the data that can be regenerated if required and removes it from the caches (datablocks), thus
    clearing up space in memory.
    ...
    Flags:
      ...
      - dirty : d                      (bool)          [create]
          If toggled then remove all heavy data that is dirty.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def getInputDeviceRange(*args, **kwargs):
    """
    This command lists the minimum and maximum values the device axis can return. This value is the raw device values before
    any mapping is applied. If you don't specify an axis the values for all axes of the device are returned.
    
    Flags:
      - maxValue : max                 (bool)          [create]
          list only the maximum value of the axisFlag can appear in Create mode of commandFlag can have multiple arguments, passed
          either as a tuple or a list.
...
def sysFile(*args, **kwargs):
    ...
    Flags:
      ...
      - removeEmptyDir : red           (bool)          [create]
          ...
          which is not empty.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple
          or a list.
...
def detachDeviceAttr(*args, **kwargs):
    ...
    Flags:
      - all : all                      (bool)          [create]
          Delete all attachments on every device.Flag can appear in Create mode of commandFlag can have multiple arguments, passed
          either as a tuple or a list.
...
def translator(*args, **kwargs):
    ...
    Flags:
      - defaultFileRule : dfr          (unicode)       [query]
          Returns the default file rule value, can return "" as wellFlag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
    
      - defaultOptions : do            (unicode)       [query]
          Return/set a string of default options used by this translator.
    
      - extension : ext                (unicode)       [query]
          Returns the default file extension for this translator.
    
      - fileCompression : cmp          (unicode)       [query]
          Specifies the compression action to take when a file is saved. Possible values are "compressed", "uncompressed"
          "asCompressed".
    
      - filter : f                     (unicode)       [query]
...
def scriptNode(*args, **kwargs):
    ...
    Flags:
      - afterScript : afterScript      (unicode)       [create,query,edit]
          The script executed when the script node is deleted.C: The default is an empty string.Q: When queried, this flag returns
          a string.
      ...
      - ignoreReferenceEdits : ire     (bool)          [create]
          ...
          recorded as edits by default.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as
          a tuple or a list.
      ...
      - scriptType : st                (int)           [create,query,edit]
          ...
          render5Execute on software frame render6Execute on scene configuration7Execute on time changedC: The default value is
          0.Q: When queried, this flag returns an int.
...
def dgmodified(*args, **kwargs):
    """
    The dgmodifiedcommand is used to find out which nodes in the dependency graph have been modified. This is mostly useful
    for fixing instances where file new asks you to save when no changes have been made to the scene.
...
def openGLExtension(*args, **kwargs):
    ...
    Flags:
      ...
      - vendor : vnd                   (bool)          [create]
          Specifies to query the company responsible for the OpenGL implementation.Flag can appear in Create mode of commandFlag
...
def whatsNewHighlight(*args, **kwargs):
    ...
    Flags:
      ...
      - showStartupDialog : ssd        (bool)          [create,query]
          Set whether the settings dialog for this feature appears on startup.Flag can appear in Create mode of commandFlag can
          have multiple arguments, passed either as a tuple or a list.
...
def autoSave(*args, **kwargs):
    ...
    Flags:
      ...
      - destinationFolder : df         (bool)          [query]
          ...
          relative to the workspace root). The returned string will end with a trailing separator ('/').Flag can appear in Create
...
def setInputDeviceMapping(*args, **kwargs):
    ...
    Flags:
      ...
      - world : w                      (bool)          [create]
          translate the device coordinates into world space coordinatesFlag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def importFile(filepath, **kwargs):
    ...
    Flags:
      ...
      - groupName:
          ...
          will be grouped under.
      - renameAll:
          Only available with -import. If true, rename all newly-created nodes, not just those whose names clash with existing
          nodes. Only available with -import.
      - renamingPrefix:
          The string to use as a prefix for all objects from this file. This flag has been replaced by -ns/namespace.
      - swapNamespace:
          Can only be used in conjunction with the -r/reference or -i/import flags. This flag will replace any occurrences of a
...
def namespaceInfo(*args, **kwargs):
    ...
    are colon-separated. The namespace format flags are: "baseName"("shortName"), "fullName" and "absoluteName". The flags
    are used in conjunction with the main query flags to specify the desired namespace format of the returned result. They
    can also be used to return the different formats of a specified namespace. By default, when no format is specified, the
    ...
    Flags:
      - absoluteName : an              (bool)          [create]
          This is a general flag which can be used to specify the desired format for the namespace(s) returned by the command. The
          absolute name of the namespace is a full namespace path, starting from the root namespace ":" and including all parent
          namespaces. For example ":ns:ball" is an absolute namespace name while "ns:ball" is not. The absolute namespace name is
      ...
      - baseName : bn                  (bool)          [create]
          ...
          the base name of an object named "ns:ball" is "ball". This mode will always return the base name in the same manner, and
          is not affected by the current namespace or relative namespace mode. See also other format modifiers 'absoluteName',
      ...
      - fullName : fn                  (bool)          [create]
          ...
          representing the root namespace. For example "ns:ball" is a full name, whereas ":ns:ball" is an absolute name. This mode
          is affected by the current namespace and relative namespace modes. See also other format modifiers 'baseName',
          'absoluteName', etc.
      ...
      - internal : int                 (bool)          [create]
          ...
          include internal nodes (for example itemFilters) that are not listed by default.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def selLoadSettings(*args, **kwargs):
    ...
    will succeed and change both b and c to the loaded state: selLoadSettings -e -deferReference 0 "1" "2"; whereas the
    following command will fail and leave both b and c in the unloaded state: selLoadSettings -e -deferReference 0 "2" "1";
    Bear in mind that the following command will also change both b and c to the loaded state: selLoadSettings -e
    -deferReference 0 "1"; This is because setting a reference to the loaded state automatically sets all child references
    to the loaded state as well. And vice versa, setting a reference the the unloaded state automatically sets all child
    reference to the unloaded state.
    ...
    Flags:
      ...
      - numSettings : ns               (int)           [create,query]
          ...
          scene plus 1.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
...
def fileBrowserDialog(*args, **kwargs):
    ...
    Flags:
      ...
      - dialogStyle : ds               (int)           [create]
          0 for old style dialog1 for Windows 2000 style Explorer style2 for Explorer style with "Shortcut" tip at bottom
    
      - fileCommand : fc               (script)        [create]
          The script to run on command action
    
      - fileType : ft                  (unicode)       [create]
          Set the type of file to filter. By default this can be any one of: "mayaAscii", "mayaBinary", "mel", "OBJ", "directory",
          "plug-in", "audio", "move", "EPS", "Illustrator", "image".plug-ins may define their own types as well.
      ...
      - windowTitle : wt               (unicode)       [create]
          Set the window title of a style 1 or 2 dialog boxFlag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def listDeviceAttachments(*args, **kwargs):
    ...
    Flags:
      ...
      - file : f                       (unicode)       [create]
          Specify the name of the file to write out device attachments.Flag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def cacheFile(*args, **kwargs):
    ...
    f/fileName flag. If no file name is provided, the cacheFile name is prefixed with "backup" followed by a unique number.
    ...
    Flags:
      ...
      - cacheFormat : cf               (unicode)       [create,query]
          Cache file format, default is Maya's .mcx format, but others available via pluginFlag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
      ...
      - inTangent : it                 (unicode)       [create]
          ...
          ist/interpStartTime and iet/interpEndTime flags. Valid values are "linear", "smooth" and "step".
      ...
      - replaceWithoutSimulating : rws (bool)          [edit]
          ...
          simulating or from advancing time and letting the simulation run. This flag is valid only when neither the startTime nor
          endTime flags are used or when both the startTime and endTime flags specify the same time value.
      ...
      - worldSpace : ws                (bool)          [create]
          ...
          The expected use of this flag is for cache export.
...
def diskCache(*args, **kwargs):
    ...
    Flags:
      ...
      - cacheType : ct                 (unicode)       [create,query]
          Specifies the type of cache to overwrite. "mcfp" for particle playback cache, "mcfi" for particle initial cache. "mcj"
          for jiggle cache. This option is only activated during the cache creation.
    
      - close : c                      (unicode)       [create,query]
          Close the cache given the disk cache node name. If -eco/enabledCachesOnly is "true" only enabled disk cache nodes are
          affected.
      ...
      - delete : d                     (unicode)       [create,query]
          Delete the cache given the disk cache node name. If -eco/enabledCachesOnly is "true" only enabled disk cache nodes are
          affected.
      ...
      - empty : e                      (unicode)       [create,query]
          Clear the content of the disk cache with the given disk cache node name. If -eco/enabledCachesOnly is "true" only
      ...
      - enabledCachesOnly : eco        (bool)          [create,query]
          When present, this flag restricts the -ea/emptyAll, so that only "enabled" disk caches (i.e., disk cache nodes with the
      ...
      - tempDir : tmp                  (bool)          [create,query]
          Query-only flag for the location of temporary diskCache files.Flag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def getModulePath(*args, **kwargs):
    ...
    Flags:
      - moduleName : mn                (unicode)       [create]
          The name of the module whose path you want to retrieve.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def fileDialog(*args, **kwargs):
    ...
    Flags:
      ...
      - directoryMask : dm             (unicode)       [create]
          This can be used to specify what directory and file names will be displayed in the dialog. If not specified, the current
          directory will be used, with all files displayed. The string may contain a path name, and must contain a wild-carded
          file specifier. (eg "\*.cc" or "/usr/u/\*") If just a path is specified, then the last directory in the path is taken to
          be a file specifier, and this will not produce the desired results.
      ...
      - title : t                      (unicode)       [create]
          Set title text. The default value under "write" mode is "Save As". The default value under "read" mode is "Open".Flag
...
def ogs(*args, **kwargs):
    ...
    Flags:
      ...
      - reloadTextures : rlt           (bool)          [create]
          If used then reload all textures for OGS.Flag can appear in Create mode of commandFlag can have multiple arguments,
          passed either as a tuple or a list.
      ...
      - reset : r                      (bool)          [create,query]
          ...
          be affected is returned but the reset is not actually done.
...
def dagObjectCompare(*args, **kwargs):
    """
    dagObjectCompare can be used to compare to compare objects based on: type - Currently supports transform nodes and shape
    nodesrelatives - Compares DAG objects' children and parentsconnections - Checks to make sure the two dags are connected
    to the same sources and destinationsattributes - Checks to make sure that the properties of active attributes are the
    same
    ...
    Flags:
      ...
      - short : s                      (bool)          [create]
          Compress output to short form (not as verbose)Flag can appear in Create mode of commandFlag can have multiple arguments,
          passed either as a tuple or a list.
...
def deviceEditor(*args, **kwargs):
    ...
    Flags:
      ...
      - takePath : tp                  (unicode)       [query,edit]
          The path used for writing/reading take data through the editor.Flag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def mouse(*args, **kwargs):
    ...
    Flags:
      ...
      - mouseButtonTrackingStatus : mbs (bool)          [create]
          returns the current number of mouse buttons being tracked.Flag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def memory(*args, **kwargs):
    ...
    Flags:
      ...
      - swapLogical : swl              (bool)          [create]
          Returns size of logical swapFlag can appear in Create mode of commandFlag can have multiple arguments, passed either as
          a tuple or a list.
...
def timer(*args, **kwargs):
    ...
    -s; timer -s -name "innerTimer"; timer -e -name "innerTimer"; timer -e; I the -e flag or -lap flag return the time
    elapsed since the last 'timer -s' call.I the -s flag has no return value.
    ...
    Flags:
      ...
      - name : n                       (unicode)       [create]
          Use a named timer for the operation. If this is omitted then the default timer is assumed.Flag can appear in Create mode
...
def imfPlugins(*args, **kwargs):
    ...
    Flags:
      ...
      - multiFrameSupport : mfs        (unicode)       [create,query]
          multi frame IO is supportedFlag can appear in Create mode of commandFlag can have multiple arguments, passed either as a
          tuple or a list.
...
def convertUnit(*args, **kwargs):
    ...
    Flags:
      - fromUnit : f                   (unicode)       [create]
          The unit to convert from. If not supplied, it is assumed to be the system default. The from unit may also be supplied as
          part of the value e.g. 11.2m (11.2 meters).
    
      - toUnit : t                     (unicode)       [create]
          The unit to convert to. If not supplied, it is assumed to be the system defaultFlag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def openMayaPref(*args, **kwargs):
    ...
    Flags:
      - errlog : el                    (bool)          [create,query,edit]
          ...
          "OpenMayaErrorLog" will be created in Maya's current working directory. Each time an API method fails, a detailed
          description of the error will be written to the file along with a mini-stack trace that indicates the routine that
          called the failing method. Defaults to false(off).Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
      ...
      - oldPluginWarning : ow          (bool)          [create,query,edit]
          ...
          and possibly incompatible Maya release. Defaults to true(on).
...
def dgdirty(*args, **kwargs):
    ...
    evaluation problems. If no nodes are specified then the current selection list is used.
    
    Flags:
      - allPlugs : a                   (bool)          [create]
          Ignore the selected or specified objects and dirty (or clean) all plugs.Flag can appear in Create mode of commandFlag
          can have multiple arguments, passed either as a tuple or a list.
...
def allNodeTypes(*args, **kwargs):
    ...
    Flags:
      - includeAbstract : ia           (bool)          [create]
          ...
          suffix "(abstract)" appended to them in the list.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def filePathEditor(*args, **kwargs):
    ...
    command is used to get the information about the file paths and modify them in bulk. Due to the fact that user may want
    to see the most frequently-used types of files, file path editor will only list four default types when user use it at
    the first time. Except from the four default types of files( the files which are used on texture, reference, audio and
    image plane ), user need to use the flag "registerType" to tell the command that which type of attribute ( must be
    "usedAsFileName" ) or of node ( only "reference" and plugin node ) the command should take care of as well, such as
    "brush.flowerImage" or "reference". User can also use the flag 'deregisterType' to tell the command that they do not
    want it to manage the specified type any more. After user register the attributes, they can use the command to query the
    files and the plugs which own them in the scene( or they can register nothing, only the default types of files will be
    listed then ). The command can list the information of each file, including its directory, the file name and if the file
    exists. The command can also list the information of the associated node/plug, including its name, the type name and the
    type label. Besides, user can use the command to edit each file path. User can use different flags to do different kinds
    of remapping, such as look for a file with the same name in the target directory or copy the file to a directory and
    remap the path to it(please see flags "repath", "copyAndRepath" and "replaceField"). All the results of these operations
    can be previewed, user can know the final state of each path through the returned values(please see flag "preview"). In
    ...
    Flags:
      ...
      - copyAndRepath : cr             (unicode, unicode) [create]
          ...
          name as the original file of the plug. The command will look for the file from the source location first. If no
          appropriate file found, the command will try to use the original file of the plug. If the file is still absent, nothing
          will happen then.
    
      - deregisterType : dt            (unicode)       [create]
          Deregister file type from the registered list to indicate that the command do not need to manage that kind of files and
          remove the saved types in the preference file to disable auto-loading type. To deregister a type temporarily, please use
          the flag "temporary". Then the type will still be auto-loaded. The flag cannot remove an unregistered type. However, if
          a type has been deregistered temporarily, user can deregister it without "temporary" to remove the saved type in the
          preference file.
    
      - force : f                      (bool)          [create]
          Used with flag "repath" to repath all files to the new location, including the resolved files. Otherwise, "repath" will
          only deal with the missing files. Used with flag "copyAndRepath" to overwrite the clash file at the destination.
          Otherwise, "copyAndRepath" will use the existing file at the destination instead of overwriting it. The default value is
          off.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
    
      - listDirectories : ld           (unicode)       [query]
          List all sub directories of the specified directory, the sub directories must have one or more files and the types of
          these files have been registered. If no specified directory is provided, all appropriate directories in the scene will
          be returned.
      ...
      - listRegisteredTypes : lrt      (bool)          [query]
          Query registered attribute types. The registered types include the auto-loaded types from the preference file and the
          manually registered types from user(both with and without "temporary").
    
      - preview : p                    (bool)          [create]
          Used with "repath", "replaceString" or "copyAndRepath" to preview the result of the operation instead of excuting it.
          When it is used with "repath" or "replaceString", the command returns the new file path and the information that if the
          new file exists or not. The path name and the file status are listed in pairs. When it is used with "copyAndRepath", the
          command returns the files which need copying.
    
      - recursive : rc                 (bool)          [create]
          Used with flag "repath" to search the files in the target directory and its sub directories recursively. If the flag is
          on, the command will repath the plug to the file which has the same name in the target directory or the sub directories.
          If the flag is off, the command will only change the file path and replace the directory part of the file with the new
          target directory no matter if the new file exists or not.
    
      - refresh : rf                   (bool)          [create]
          Clear and re-collect the file information in the scene. Due to the fact that the command cannot know if user has
          modified some file paths in the scene, user need to use this flag to ask the command to update the file information.
    
      - registerType : rt              (unicode)       [create]
          Register a new file type to make the command be able to handle this kind of files and save the type in the preference
          file to auto-load the type when user restart Maya and file path editor. The flag cannot deal with an occupied type or an
          occupied label which means the type has been registered/saved and it is using another label or the label has been
          registered/saved and it is used by another type. However, if a type and its label is registered temporarily (please see
          the flag "temporary"), user can register them again without "temporary" to save it in the preference file. Then the type
          will be auto-loaded as well.
    
      - relativeNames : rel            (bool)          [query]
          Used with "listDirectories" or "listFiles" to return the relative path of each directory or each file. If the file or
          the directory is not in the current project folder, the returned path will still be a full path.
      ...
      - replaceAll : ra                (bool)          [create]
          Used with flag "replaceString", specifies how many times the matched string can be replaced. When the flag is false,
          only the first matched string will be replaced. Otherwise, all matched strings will be replaced. The default value of
          the flag is false.
    
      - replaceField : rfd             (unicode)       [create]
          Used with flag "replaceString" to control the scope of replacement. Only the matched string in the associated part can
          be replaced: "pathOnly" - only the strings in the directory part "nameOnly" - only the strings in the file name(no
          directory) "fullPath" - any strings in the full name The default argument of the flag is "fullPath".
      ...
      - replaceString : rs             (unicode, unicode) [create]
          ...
          target string and the second one is the new string. Used with "replaceField" or "replaceAll" together to control how the
          replacement need to be performed.
    
      - status : s                     (bool)          [query]
          Used with "listFiles" to query the existence of one or more files under a directory. The returned status will follow its
          associated file in the returned list. (please see the sample of "listFiles")
    
      - temporary : tmp                (bool)          [create]
          Used to make the flag "register"/"deregister" not to save any changes in the preference file. The saved types in
          preference file will be auto-loaded when user restart Maya. If this flag is on, "register"/"deregister" will only affect
          the loaded types in Maya. And the temporary types will not be loaded next time user open Maya.
    
      - typeLabel : tl                 (unicode)       [create,query]
          Used with 'registerType' to set the label name for the new file type. Used with 'query' to return the type label of the
      ...
      - withAttribute : wa             (bool)          [query]
          Used with "listFiles" to return the plug which is using the file. For example, if "file.jpg" is used by the plug
          "node1.fileTextureName", then the returned string will become an ordered pair : "file.jpg node1.fileTextureName".
...
def assignInputDevice(*args, **kwargs):
    """
    This command associates a command string (i.e. a mel script) with the input device. When the device moves or a button on
    the device is pressed, the command string is executed as if you typed it into the window. If the command string contains
    the names of buttons or axes of the device, the current value of these buttons/axes are substituted in. Buttons are
    reported as booleans and axes as doubles. This command is most useful for associating buttons on a device with commands.
    For using a device to capture continous movements it is much more efficient to attach the device directly into the
    dependency graph.
    ...
    Flags:
      ...
      - immediate : im                 (bool)          [create]
          Immediately executes the command, without using the queue.Flag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
      ...
      - multiple : m                   (bool)          [create]
          ...
          command string is attached to the device, all other command strings are deleted.
...
def date(*args, **kwargs):
    ...
    Flags:
      ...
      - format : f                     (unicode)       [create]
          ...
          replaced with the corresponding values: YYYYYYMMDDhhmmss KeywordBecomesCurrent year, using 4 digitsLast two digits of
          the current yearCurrent month, with leading 0 if necessaryCurrent day, with leading 0 if necessaryCurrent hour, with
          leading 0 if necessaryCurrent minute, with leading 0 if necessaryCurrent second, with leading 0 if necessaryFlag can
...
def exportEdits(*args, **kwargs):
    ...
    Flags:
      ...
      - excludeHierarchy : ehr         (bool)          [create,query]
          By default exporting passes all DAG parents and DAG history to the export file. To prevent any DAG relations not
          otherwise connected to the target nodes to be output, specify the -excludeHierarchy flag.
      ...
      - excludeNode : en               (unicode)       [create,query]
          ...
          nodes that might otherwise be exported. In the case where more than one Maya node has the same name, you can specify the
          DAG path to uniquely identify the node.
      ...
      - exportSelected : exs           (bool)          [create,query]
          ...
          exported nodes will be exported. Any default nodes (the internal nodes that Maya creates on startup) that are selected
          are skipped. Please note that when using the exportSelected flag, only selected nodes are exported, and
      ...
      - includeNode : includeNode      (unicode)       [create,query]
          ...
          the same name, you can specify the DAG path to uniquely identify the node.
      ...
      - selected : sel                 (bool)          [create,query]
          ...
          selected nodes are not exported, only the edits on them, and any nodes found via the include flags the are used (such as
          includeAnimation, includeNetwork and so on).
    
      - type : typ                     (unicode)       [create,query]
          Set the type of the exported file. Valid values are "editMA" or "editMB". Note that this command respects the global
          "defaultExtensions" setting for file naming that is controlled with the file command defaultExtensions option. See the
...
def requires(*args, **kwargs):
    ...
    I/O, so users should not have any need to use this command themselves. The flags "-nodeType" and "-dataType" specify the
    node types and data types defined by the plug-in. When Maya open a scene file, it runs "requires" command in the file
    and load required plug-ins. But some plug-ins may be not loaded because they are missing. The flags "-nodeType" and
    "-dataType" are used by the missing plug-ins. If one plug-in is missing, nodes and data created by this plug-in are
    created as unknown nodes and unknown data. Maya records their original types for these unknown nodes and data. When
    these nodes and data are saved back to file, it will be possible to determine the associated missing plug-ins. And when
    export selected nodes, Maya can write out the exact required plug-ins. The flags "-nodeType" and "-dataType" is
    optional. In this command, if these flags are not given for one plug-in and the plug-in is missing, the "requires"
    ...
    Flags:
      - dataType : dt                  (unicode)       [create]
          ...
          plug-in.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def hardware(*args, **kwargs):
    ...
    Flags:
      ...
      - numProcessors : npr            (bool)          [create]
          Returns string identifying the number of processorsFlag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def error(*args, **kwargs):
    ...
    the event of runtime errors. The string argument is displayed in the command window (or stdout if running in batch mode)
    after being prefixed with an error message heading and surrounded by //. The error command also causes execution to
    terminate with an error. Using error is like raising an exception because the error will propagate up through the call
    chain. You can use catch to handle the error from the caller side. If you don't want execution to end, then you probably
    want to use the warning command instead.
    ...
    Flags:
      - showLineNumber : sl            (bool)          [create]
          ...
          display instead.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
...
def dgInfo(*args, **kwargs):
    ...
    upstream from it) are propagated to all plugs depending on it (e.g. downstream connections or "affected" outputs).DIRTY
    ...
    downstream/affected plugs.DIRTY PROPis a less common but still valid state for a plug whose upstream values have changed
    but no evaluation request has come through for that plug's value. Like the DIRTY BLOCKstate it will trigger an evaluaton
    when it's value is requested. Where it differs is that when another change comes through from an upstream plug this plug
    will (again) propagate the dirty message to all downstream/affected plugs. You will only see this state when the DG was
    not certain that all downstream plugs were notified of their dirty status the last time this plug was marked dirty
    itself (e.g. if the dirty propagation was intercepted by a node or the graph connections changed since the last dirty
    message).CLEAN BLOCKshould never be seen in a valid DG. This indicates that while the value of the plug is clean (i.e.
    valid) it will not propagate a dirty state when its value changes. That means downstream nodes will not be notified that
    the graph is changing and they will not evaluate properly. Recovering from this invalid state requires entering the
    command dgdirty -ato mark everything dirty and restart proper evaluation. (Think of this command as the CTL-ALT-DELof
    the DG world.)
    ...
    Flags:
      ...
      - island : island                (bool)          [create]
          Print detailed debug information about the island for each node. Note that this flag will typically generate a large
          amount of debug information, so it is recommended that the list of nodes passed to the command be brief when using the
          islandDetail flag. The information includes the full list of nodes in the island, as well as all islands connected to
          the island.
      ...
      - outputFile : of                (unicode)       [create]
          Send the output to the file FILE instead of STDERRFlag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def cmdFileOutput(*args, **kwargs):
    ...
    Flags:
      ...
      - closeAll : ca                  (bool)          [create]
          Closes all open files.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a
          tuple or a list.
      ...
      - status : s                     (int)           [create,query]
          ...
          indicates an error condition, 0 indicates file is ready for writing.
...
def dbcount(*args, **kwargs):
    ...
    Flags:
      ...
      - file : f                       (unicode)       [create]
          Destination file of the enabled count objects. Use the special names stdoutand stderrto redirect to your command window.
          As well, the special name msdevis available on NT to direct your output to the debug tab in the output window of
          Developer Studio.
      ...
      - spreadsheet : s                (bool)          [create]
          ...
          Level1 Level2 Level3...', making the data suitable for opening directly in a spreadsheet table.Flag can appear in Create
          mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def saveImage(*args, **kwargs):
    ...
    Flags:
      ...
      - sceneFile : sf                 (unicode)       [edit]
          The name of the file that the icon is to be associated with.Flag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def recordAttr(*args, **kwargs):
    """
    This command sets up an attribute to be recorded. When the record command is executed, any changes to this attribute are
    recorded. When recording stops these changes are turned into keyframes. If no attributes are specified all attributes of
    the node are recorded. In query mode, return type is based on queried flag.
    ...
    Flags:
      ...
      - delete : d                     (bool)          [create]
          Do not record the specified attributesFlag can appear in Create mode of commandFlag can have multiple arguments, passed
          either as a tuple or a list.
...
def preloadRefEd(*args, **kwargs):
    ...
    Flags:
      ...
      - selectFileNode : sf            (bool)          [query]
          ...
          an argument to the selLoadSettings command.Flag can appear in Create mode of commandFlag can have multiple arguments,
          passed either as a tuple or a list.
...
def unassignInputDevice(*args, **kwargs):
    ...
    Flags:
      - clutch : c                     (unicode)       [create]
          Only delete command attachments with this clutch.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def pluginDisplayFilter(*args, **kwargs):
    ...
    Flags:
      ...
      - listFilters : lf               (bool)          [query]
          Returns an array of all plugin display filters.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def melInfo(*args, **kwargs):
    ...
    query the definition of each MEL procedure using the "whatIs" command.
...
def launch(*args, **kwargs):
    ...
    Flags:
      - directory : dir                (unicode)       [create]
          A directory.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
      ...
      - movie : mov                    (unicode)       [create]
          ...
          end with .mpg, .mpeg, .wmv, .mov, or .qt.
...
def displayString(*args, **kwargs):
    ...
    Flags:
      ...
      - keys : k                       (bool)          [create,query]
          ...
          string. The command will return a list of all identifier keys that contain this identifier string as a substring.Flag
          can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def pluginInfo(*args, **kwargs):
    """
    This command provides access to the plugin registry of the application. It is used mainly to query the characteristics
    of registered plugins. Plugins automatically become registered the first time that they are loaded. The argument is
    either the internal name of the plug-in or the path to access it.
    
    Flags:
      - activeFile : af                (bool)          [query]
          Specifies that the -pluginsInUse/-pu flag should return the plugins used by the active file only, not the entire scene.
          For use during export selected.
    
      - apiVersion : av                (bool)          [query]
          returns a string containing the version of the API that this plugin was complied with. See the comments in MTypes.h for
          the details on how to interpret this value.
      ...
      - changedCommand : cc            (script)        [create]
          adds a callback that will get executed every time the plugin registry changes. Any other previously registered callbacks
          will also get called.
    
      - command : c                    (unicode)       [query]
          returns a string array containing the names of all of the normal commands registered by this plugin. Constraint,
          control, context and model editor commands are not included.
    
      - constraintCommand : cnc        (unicode)       [query]
          Returns a string array containing the names of all of the constraint commands registered by this plugin.Flag can appear
          in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
      ...
      - unloadOk : uo                  (bool)          [query]
          returns a boolean that specifies whether or not the plugin can be safely unloaded. It will return false if the plugin is
          currently in use. For example, if the plugin adds a new dependency node type, and an instance of that node type is
...
def launchImageEditor(*args, **kwargs):
    ...
    Flags:
      ...
      - viewImageFile : vif            (unicode)       [create]
          Opens up an Image editor to view images.Flag can appear in Create mode of commandFlag can have multiple arguments,
          passed either as a tuple or a list.
...
def loadPlugin(*args, **kwargs):
    ...
    name with the extension removed. For example, if you load the plug-in "newNode.mll" the name used in the Maya's registry
    will be "newNode". This value as well as that value with either a ".so", ".mll" or ".bundle" extension can be used as
    valid arguments to either the unloadPlugin or pluginInfo commands.
    ...
    Flags:
      - addCallback : ac               (script)        [create]
          ...
          string, or a Python callable object such as a function. If you specify a string, then put the formatting specifier "%s"
          where you want the name of the plug-in to be inserted. If you specify a callable such as a function, then the name of
      ...
      - removeCallback : rc            (script)        [create]
          Removes a procedure which was previously added with -addCallback.Flag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def dbpeek(*args, **kwargs):
    ...
    Flags:
      - allObjects : all               (bool)          [create,query]
          Ignore any specified or selected objects and peek into all applicable objects. The definition of "allObjects" will vary
          based on the type of peek being performed - see the flag documentation for details on what it means for a given type. By
          default if no objects are selected or specified then it will behave as though this flag were set.
      ...
      - operation : op                 (unicode)       [create,query]
          Specify the peeking operation to perform. The various types are registered at run time and can be listed by querying
          this flag without a value. If you query it with a value then you get the detail values that peek type accepts and a
          description of what it does. In query mode, this flag can accept a value.Flag can appear in Create mode of commandFlag
          can have multiple arguments, passed either as a tuple or a list.
      ...
      - type : t                       (unicode)       [create,query]
          Specify the type of operation desired in string form. The acceptable values for the type string are documented in the
          flag to which they will be applied.
...
def internalVar(*args, **kwargs):
    ...
    Flags:
      ...
      - userWorkspaceDir : uwd         (bool)          [create]
          Return the user workspace directory (also known as the projects directory).Flag can appear in Create mode of commandFlag
          can have multiple arguments, passed either as a tuple or a list.
...
def undoInfo(*args, **kwargs):
    ...
    Flags:
      ...
      - stateWithoutFlush : swf        (bool)          [create,query]
          Turns undo/redo on or off without flushing the queue. Use with CAUTION!! Note that if you perform destructive operations
          while stateWithoutFlush is disabled, and you then enable it again, subsequent undo operations that try to go past the
          destructive operations may be unstable since undo will not be able to properly reconstruct the former state of the
          scene.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def fileDialog2(*args, **kwargs):
    ...
    Flags:
      ...
      - optionsUICommit : ocm          (unicode)       [create]
          ...
          the parent layout into which controls have been added using the optionsUICreate flag.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
      ...
      - selectionChanged : sc          (unicode)       [create]
          MEL only. The string is interpreted as a MEL callback, to be called when the user changes the file selection in the file
          dialog. The callback is of the form: global proc MyCustomSelectionChanged(string $parent, string $selection) The parent
          argument is the parent layout into which controls have been added using the optionsUICreate flag. The selection argument
          is the full path to the newly-selected file.
...
def timerX(*args, **kwargs):
    ...
    Flags:
      - startTime : st                 (float)         [create]
          When this flag is used, the command returns the elapsed time since the specified start time.Flag can appear in Create
          mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def referenceQuery(*args, **kwargs):
    ...
    open A.ma and, instead of simply parenting pSphere1, you were to instance it. While A.ma is open, "B:C:pSphere1" may now
    be an amibiguous name, replaced by "|B:C:pSphere1" and "group1|B:C:pSphere1". However querying the edits which affect
    C.ma would still return a setAttr edit affecting "C:pSphere1" since it was applied prior to B:C:pSphere1 being
    instanced. Some tips: 1. Use the '-topReference' flag to query only those edits which were applied from the currently
    open file. 2. Use the '-onReferenceNode' flag to limit the results to those edits where are stored on a given reference
    node. You can then use various string manipulation techniques to extrapolate the current name of any affected nodes.
    ...
    Flags:
      ...
      - editAttrs : ea                 (bool)          [create]
          ...
          attributes involved in the reference edit will be returned. If an edit involves multiple attributes (e.g. "connectAttr"
          edits) the nodes will be returned as separate, consecutive entries in the string array. A valid target is either a
          reference node, a reference file, or a referenced node. If a referenced node is specified, only those edits which affect
          that node will be returned. If a reference file or reference node is specified any edit which affects a node in that
          reference will be returned. If no target is specified all edits are returned. This command can be used on both loaded
          and unloaded references. By default it will return all the edits, formatted as MEL commands, which apply to the target.
          This flag can be used in combination with the '-ea/-editAttrs' flag to indicate that the names of both the involved
          nodes and attributes will be returned in the format 'node.attribute'.
      ...
      - editNodes : en                 (bool)          [create]
          Returns string array. A main flag used to query the edits that have been applied to the target. Only the names of the
          nodes involved in the reference edit will be returned. If an edit involves multiple nodes (e.g. "connectAttr" edits) the
      ...
      - shortName : shn                (bool)          [create]
          ...
          namespace returned will be the base name of the namespace. (i.e. the base name of the full namespace path ":AAA:BBB:CCC"
          is "CCC" )
...
def cacheFileTrack(*args, **kwargs):
    ...
    be used to modify the track state, for example, to lock or mute a track. In query mode, return type is based on queried
    flag.
    ...
    Flags:
      ...
      - removeEmptyTracks : ret        (bool)          [create]
          This flag is used to remove all tracks that have no clips.Flag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def attrCompatibility(*args, **kwargs):
    ...
    compatibility changes to files.The first optional command argument argument is a node name and the second optional
    command argument is an attribute name.Warning:Only use this command to describe changes in names or attributes of nodes
    that youhave written as plugins. Do notuse this command to change information about builtin dependency graph nodes.
    Removing attributes on a plug-in node is a special case. Use a separate attrCompatibility call with pluginNode flag and
    name so that these attributes can be tracked even though the plug-in may not be loaded.
    ...
    Flags:
      ...
      - pluginNode : pn                (unicode)       [create]
          ...
          reference node attributes of unloaded plug-ins. Specifically, this works in the case when attributes are being
          removed.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
