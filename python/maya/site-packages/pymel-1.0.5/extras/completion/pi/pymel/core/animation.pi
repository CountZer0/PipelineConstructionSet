...
def flexor(*args, **kwargs):
    ...
    Flags:
      ...
      - name : n                       (unicode)       [create]
          This flag is obsolete.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a
          tuple or a list.
...
def keyingGroup(*args, **kwargs):
    """
    This command is used to manage the membership of a keying group. Keying groups allow users to effectively manage related
    keyframe data, allowing keys on attributes in the keying group to be set and edited as a single entity.
    ...
    Flags:
      ...
      - addElement : add               (PyNode)        [edit]
          Adds the list of items to the given set. If some of the items cannot be added to the set because they are in another set
          which is in the same partition as the set to edit, the command will fail.
      ...
      - include : include              (PyNode)        [edit]
          ...
          is a less strict version of the -add/addElement operation.
      ...
      - minimizeRotation : mr          (bool)          [create,query,edit]
          ...
          keyingGroups. Those will need to set minimizeRotation on their respective keyingGroupsFlag can appear in Create mode of
      ...
      - renderable : r                 (bool)          [create,query]
          ...
          geometry. These sets are referred to as shading groups and are automatically connected to the "renderPartition" node
          when created (to ensure mutual exclusivity of the set's members with the other sets in the partition). This flag is for
          use in creation or query mode only. The default value is false which means a normal set is created.
      ...
      - setActiveFilter : fil          (unicode)       [create,query,edit]
          ...
          categories that match the filter will be activated. If the setActiveFilter is set to "NoKeyingGroups", keying groups
          will not be activated at all. If the setActiveFilter is set to "AllKeyingGroups", we will activate any keying group
          rather than just those with matching categories.
...
def setDrivenKeyframe(*args, **kwargs):
    ...
    is connected like this:driver attribute ---animCurve ---driven attributeYou can set driven keyframes with more than a
    ...
    Flags:
      ...
      - controlPoints : cp             (bool)          [create]
          Explicitly specify whether or not to include the control points of a shape (see "-s" flag) in the list of attributes.
          Default: false.
      ...
      - inTangentType : itt            (unicode)       [create]
          The in tangent type for keyframes set by this command. Valid values are "spline", "linear", "fast", "slow", "flat",
          "stepped", "step next", "fixed", "clamped" and "plateau". Default is "keyTangent -q -g -inTangentType"
      ...
      - outTangentType : ott           (unicode)       [create]
          The out tangent type for keyframes set by this command. Valid values are "spline", "linear", "fast", "slow", "flat",
          "stepped", "step next", "fixed", "clamped" and "plateau". Default is "keyTangent -q -g -outTangentType"
    
      - shape : s                      (bool)          [create]
          Consider attributes of shapes below transforms as well, except "controlPoints". Default: true
    
      - value : v                      (float)         [create]
          Value to set the keyframe at. Default is the current value.Flag can appear in Create mode of commandFlag can have
...
def animView(*args, **kwargs):
    ...
    Flags:
      ...
      - nextView : nv                  (bool)          [edit]
          Switches to the next view.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a
          tuple or a list.
...
def nonLinear(*args, **kwargs):
    ...
    For example, if the type is "bend", then the flags "-curvature", "-lowBound" and "-highBound" may be used to initialize,
    edit or query those attribute values on the bend node. Examples of this are covered in the example section below.
    ...
    Flags:
      ...
      - afterReference : ar            (bool)          [create,edit]
          ...
          automatically. If the geometry being deformed is referenced then -after mode is used in adding the new deformer
          otherwise -before mode is used. The net effect when using -afterReference to build deformer chains is that internal
      ...
      - commonParent : cp              (bool)          [create]
          Creates a new transform and parents the selected object and the deformer handle under it. This flag is valid only when a
      ...
      - defaultScale : ds              (bool)          [create]
          ...
          upon creation, edited and queried as normal flags using either the long or the short attribute name. e.g.Flag can appear
...
def shot(*args, **kwargs):
    ...
    Flags:
      ...
      - transitionOutLength : tol      (time)          [create,query,edit]
          Length of the transtion out of the shot.Flag can appear in Create mode of commandFlag can have multiple arguments,
...
def normalConstraint(*args, **kwargs):
    ...
    Flags:
      ...
      - worldUpObject : wuo            (PyNode)        [create,query,edit]
          Set the DAG object use for worldUpType "object" and "objectrotation". See worldUpType for greater detail. The default
      ...
      - worldUpType : wut              (unicode)       [create,query,edit]
          ...
          "objectrotation", "vector", or "none". If the value is "scene", the upVector is aligned with the up axis of the scene
          and worldUpVector and worldUpObject are ignored. If the value is "object", the upVector is aimed as closely as possible
          to the origin of the space of the worldUpObject and the worldUpVector is ignored. If the value is "objectrotation" then
          the worldUpVector is interpreted as being in the coordinate space of the worldUpObject, transformed into world space and
          the upVector is aligned as closely as possible to the result. If the value is "vector", the upVector is aligned with
          worldUpVector as closely as possible and worldUpMatrix is ignored. Finally, if the value is "none" no twist calculation
          is performed by the constraint, with the resulting "upVector" orientation based previous orientation of the constrained
          object, and the "great circle" rotation needed to align the aim vector with its constraint. The default worldUpType is
          "vector".Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def retarget(*args, **kwargs):
    ...
    Flags:
      ...
      - lowerBody : lb                 (unicode)       [create]
          ...
          "scaledFootPlacement" and "absoluteFootPlacement". Default is scaledFootPlacement.
    
      - lowerScale : ls                (unicode)       [create]
          Specify the way scale is calculated for the lower body. Valid techniques are: "hipToFoot", "hipToToe", "overallHeight",
          and "other". This option is used when the lowerBody flag is set to anything other than "jointRotationsOnly". Default is
          hipToFoot. When "other" is used, the lowerScaleFactor flag should be used to specify the scale factor.
      ...
      - restPoseFrame : rpf            (int)           [create]
          ...
          range of frames being retargeted. This allows you to retarget without wiping out the rest pose.Flag can appear in Create
          mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
    
      - scaleHandsFrom : shf           (unicode)       [create]
          Specify where the arms are scaled from. Valid arguments are: "shoulder", "root" and "origin". This option is only used
          when the upperBody flag is set to "scaledHandPlacement". Defaults is shoulder.
      ...
      - upperBody : ub                 (unicode)       [create]
          ...
          "scaledHandPlacement", "absoluteHandPlacement" and "maintainHandDistance". Default is jointRotationsOnly.
      ...
      - upperScale : us                (unicode)       [create]
          ...
          "followLower", and "other". This option is used when the upperBody flag is set to anything other than
          "jointRotationsOnly". Defaults is shoulderToHand. When "other" is used, the upperScaleFactor flag should be used to
...
def scaleKey(*args, **kwargs):
    ...
    animation curves. The animation curves comprising a keyset depend on the value of the "-animation" flag: keysOrObjects:
    Any active keys, when no target objects or -attribute flags appear on the command line, orAll animation curves connected
    to all keyframable attributes of objects specified as the command line's targetList, when there are no active keys.keys:
    Only act on active keys or tangents. If there are no active keys or tangents, don't do anything.objects: Only act on
    specified objects. If there are no objects specified, don't do anything.Note that the "-animation" flag can be used to
    override the curves uniquely identified by the multi-use "-attribute" flag, which takes an argument of the form
    attributeName, such as "translateX". Keys on animation curves are identified by either their time values or their
    indices. Times and indices can be given individually or as part of a list or range. -time 10palmeans the key at frame 10
    (PAL format).-time 1.0sec -time 15ntsc -time 20means the keys at time 1.0 second, frame 15 (in NTSC format), and time 20
    (in the currently defined global time unit).-time "10:20"means all keys in the range from 10 to 20, inclusive, in the
    current time unit.Omitting one end of a range means "go to infinity", as in the following examples: -time "10:"means all
    keys from time 10 (in the current time unit) onwards.-time ":10"means all keys up to (and including) time 10 (in the
    current time unit).-time ":"is a short form to specify all keys.-index 0means the first key of each animation curve.
    (Indices are 0-based.)-index 2 -index 5 -index 7means the 3rd, 6th, and 8th keys.-index "1:5"means the 2nd, 3rd, 4th,
    5th, and 6th keys of each animation curve.This command takes keyframes at (or within) the specified times (or time
    ranges) and scales them. If no times are specified, the scale is applied to active keyframes or (if no keys are active)
    all keys of active objects.
    
    Flags:
      - animation : an                 (unicode)       [create]
          Where this command should get the animation to act on. Valid values are "objects," "keys," and "keysOrObjects" Default:
      ...
      - controlPoints : cp             (bool)          [create]
          This flag explicitly specifies whether or not to include the control points of a shape (see "-s" flag) in the list of
      ...
      - includeUpperBound : iub        (bool)          [create]
          ...
          flag is a time range with a lower and upper bound. (When used with the "pasteKey" command, this flag refers only to the
          time range of the target curve that is replaced, when using options such as "replace," "fitReplace," or "scaleReplace."
      ...
      - scaleSpecifiedKeys : ssk       (bool)          [create]
          ...
          The default is true.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple
          or a list.
    
      - shape : s                      (bool)          [create]
          Consider attributes of shapes below transforms as well, except "controlPoints". Default: true. (Not valid for "pasteKey"
          cmd.)
...
def percent(*args, **kwargs):
    ...
    Flags:
      ...
      - multiplyPercent : mp           (bool)          [create]
          Multiply the percent value specified with the -v flag with existing percent valuesFlag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def orientConstraint(*args, **kwargs):
    ...
    orientConstraint takes as input one or more "target" DAG transform nodes to control the orientation of the single
    "constraint object" DAG transform The orientConstraint orients the constrained object to match the weighted average of
    the target world space orientations.
    ...
    Flags:
      - createCache : cc               (float, float)  [edit]
          ...
          start and end frames.The cache is useful if the constraint has multiple targets and the constraint's interpolation type
          is set to "no flip". The "no flip" mode prevents flipping during playback, but the result is dependent on the previous
      ...
      - deleteCache : dc               (bool)          [edit]
          Delete an existing interpolation cache.Flag can appear in Create mode of commandFlag can have multiple arguments, passed
      ...
      - skip : sk                      (unicode)       [create,edit]
          Specify the axis to be skipped. Valid values are "x", "y", "z" and "none". The default value in create mode is "none".
...
def readTake(*args, **kwargs):
    ...
    Flags:
      - angle : a                      (unicode)       [create]
          Sets the angular unit used in the take. Valid strings are "deg", "degree", "rad", and "radian".C: The default is the
          current user angular unit.
      ...
      - frequency : f                  (float)         [create]
          ...
          -noTimestamp flag should also be used. This flag resample, instead the data is assumed to be at the specified
          frequency.C: If the take file does not use time stamps, the default frequency is 60Hz.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
    
      - linear : l                     (unicode)       [create]
          Sets the linear unit used in the take. Valid strings are "mm", "millimeter", "cm", "centimeter", "m", "meter", "km",
          "kilometer", "in", "inch", "ft", "foot", "yd", "yard", "mi", and "mile".C: The default is the current user linear unit.
...
def joint(*args, **kwargs):
    ...
    -degreeOfFreedom xyz -name "Joint#" -position 0 0 0 -absolute -dof "xyz" -scale 1.0 1.0 1.0 -scaleCompensate true
    -orientation 0.0 0.0 0.0 -scaleOrientation 0.0 0.0 0.0 -limitX -360 360 -limitY -360 360 -limitZ -360 360 -angleX 0.0
    -angleY 0.0 -angleZ 0.0 -stiffnessX 0.0 -stiffnessY 0.0 -stiffnessZ 0.0 -limitSwitchX no -limitSwitchY no -limitSwitchZ
    no -rotationOrder xyz Those arguments can be specified in the creation mode, editied in the edit mode (-e), or queried
    in the query mode (-q).
    ...
    Flags:
      ...
      - orientJoint : oj               (unicode)       [edit]
          ...
          the joint does not have child joint, or the distance to the child joint is zero when the argument is not "none". C.
          either flag -o or -so is set.
      ...
      - secondaryAxisOrient : sao      (unicode)       [edit]
          ...
          example, a flag combination of "-oj yzx -sao yup"would result in the y-axis pointing down the bone, the z-axis oriented
      ...
      - symmetryAxis : sa              (unicode)       [create,edit]
          ...
          only used when the symmetry flag is set to True.Flag can appear in Create mode of commandFlag can have multiple
...
def clipSchedulerOutliner(*args, **kwargs):
    ...
    Flags:
      ...
      - clipScheduler : cs             (unicode)       [edit]
          Name of the clip scheduler for which to display information.Flag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
      ...
      - dragCallback : dgc             (script)        [create,edit]
          ...
          but there are two ways to specify the callback. The recommended way is to pass a Python function object as the argument.
          In that case, the Python callback should have the form: def callbackName( dragControl, x, y, modifiers ): The values of
          ...
          Python format operator. The format values are passed in a dictionary with the keys "dragControl", "x", "y", "modifiers".
          The "dragControl" value is a string and the other values are integers (eg the callback string could be "print
      ...
      - dropCallback : dpc             (script)        [create,edit]
          ...
          the callback. The recommended way is to pass a Python function object as the argument. In that case, the Python callback
          should have the form: def pythonDropTest( dragControl, dropControl, messages, x, y, dragType ): The values of these
          arguments are the same as those for the MEL version above. The other way to specify the callback in Python is to specify
          a string to be executed. In that case, the string will have the values substituted into it via the standard Python
          format operator. The format values are passed in a dictionary with the keys "dragControl", "dropControl", "messages",
          "x", "y", "type". The "dragControl" value is a string and the other values are integers (eg the callback string could be
          "print '%(dragControl)s %(dropControl)s %(messages)r %(x)d %(y)d %(type)d'")
    
      - enable : en                    (bool)          [create,query,edit]
          The enable state of the control. By default, this flag is set to true and the control is enabled. Specify false and the
          control will appear dimmed or greyed-out indicating it is disabled.
...
def pairBlend(*args, **kwargs):
    ...
    Flags:
      ...
      - input2 : i2                    (bool)          [query]
          Returns a string array of the node(s) connected to input 2.Flag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def boneLattice(*args, **kwargs):
    ...
    Flags:
      ...
      - transform : t                  (unicode)       [create]
          Specifies which dag node is being used to rigidly transform the lattice which this node is going to deform. If this flag
          is not specified an identity matrix will be assumed.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def scaleConstraint(*args, **kwargs):
    ...
    scaleConstraint takes as input one or more "target" DAG transform nodes to which to scale the single "constraint object"
    DAG transform node. The scaleConstraint scales the constrained object at the weighted geometric mean of the world space
    ...
    Flags:
      ...
      - skip : sk                      (unicode)       [create,edit]
          Specify the axis to be skipped. Valid values are "x", "y", "z" and "none". During creation, "none" is the default. This
          flag is multi-use.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or
          a list.
...
def pose(*args, **kwargs):
    ...
    Flags:
      - allPoses : ap                  (bool)          [query]
          This flag is used to query all the poses in the scene.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def defineVirtualDevice(*args, **kwargs):
    ...
    Flags:
      ...
      - usage : use                    (unicode)       [create]
          ...
          XYposRotXZYposRotXZYposRotZYXposXposYposZrotXrotYrotZFlag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def applyTake(*args, **kwargs):
    ...
    Flags:
      ...
      - reset : r                      (bool)          [create]
          ...
          used during playback.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple
          or a list.
...
def reorderDeformers(*args, **kwargs):
    ...
    Flags:
      - name : n                       (unicode)       [create]
          This flag is obsolete and is not used.Flag can appear in Create mode of commandFlag can have multiple arguments, passed
          either as a tuple or a list.
...
def hikGlobals(*args, **kwargs):
    ...
    Flags:
      - releaseAllPinning : rap        (bool)          [query,edit]
          Sets the global release all pinning hik flag. When this flag is set, all pinning states are ignored.Flag can appear in
...
def pointOnPolyConstraint(*args, **kwargs):
    ...
    pointOnPolyConstraint takes as input one or more "target" DAG transform nodes at which to position the single
    "constraint object" DAG transform node. The pointOnPolyConstraint positions the constrained object at the weighted
...
def animCurveEditor(*args, **kwargs):
    ...
    Flags:
      ...
      - constrainDrag : cd             (int)           [create,query,edit]
          ...
          determine (and override) any prior set constrained axis.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
      ...
      - resultUpdate : ru              (unicode)       [query,edit]
          Valid values: "interactive" "delayed"Controls how changes to animCurves are reflected in the result curves (if results
          are being shown). If resultUpdate is "interactive", then as interactive changes are being made to the animCurve, the
          result curves will be updated. If modelUpdate is delayed (which is the default setting), then result curves are updated
          once the final change to an animCurve has been made.
...
def mirrorJoint(*args, **kwargs):
    ...
    Flags:
      ...
      - searchReplace : sr             (unicode, unicode) [create]
          ...
          with the second specified string.Flag can appear in Create mode of commandFlag can have multiple arguments, passed
          either as a tuple or a list.
...
def listAnimatable(*args, **kwargs):
    """
    This command list the animatable attributes of a node. Command flags allow filtering by the current manipulator, or node
    type.
    ...
    Flags:
      ...
      - manipHandle : mh               (bool)          [create]
          ...
          flags are specified, output is the same as if the "-manipHandle" flag were not present.
    
      - shape : s                      (bool)          [create]
          This flag is obsolete and no longer supported.Flag can appear in Create mode of commandFlag can have multiple arguments,
          passed either as a tuple or a list.
...
def pathAnimation(*args, **kwargs):
    ...
    Flags:
      ...
      - endTimeU : etu                 (time)          [query]
          ...
          a time.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
      ...
      - worldUpObject : wuo            (PyNode)        [create,query,edit]
          Set the DAG object to use for worldUpType "object" and "objectrotation". See -wut/worldUpType for greater detail. The
          default value is no up object, which is interpreted as world space.
    
      - worldUpType : wut              (unicode)       [create,query,edit]
          Set the type of the world up vector computation. The worldUpType can have one of 5 values: "scene", "object",
          "objectrotation", "vector", or "normal". If the value is "scene", the upVector is aligned with the up axis of the scene
          and worldUpVector and worldUpObject are ignored. If the value is "object", the upVector is aimed as closely as possible
          to the origin of the space of the worldUpObject and the worldUpVector is ignored. If the value is "objectrotation" then
          the worldUpVector is interpreted as being in the coordinate space of the worldUpObject, transformed into world space and
          the upVector is aligned as closely as possible to the result. If the value is "vector", the upVector is aligned with
          worldUpVector as closely as possible and worldUpObject is ignored. Finally, if the value is "normal" the upVector is
...
def poleVectorConstraint(*args, **kwargs):
    ...
    targets. An poleVectorConstraint takes as input one or more "target" DAG transform nodes at which to aim pole vector for
    an IK handle using the rotate plane solver. The pole vector is adjust such that the in weighted average of the world
    ...
    Flags:
      - layer : l                      (unicode)       [create,edit]
          Specify the name of the animation layer where the constraint should be added.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def keyframeOutliner(*args, **kwargs):
    ...
    Flags:
      ...
      - display : dsp                  (unicode)       [query,edit]
          narrow | wide What columns to display. When "narrow", time and value will be displayed, when "wide" tangent information
          will be displayed as wellFlag can appear in Create mode of commandFlag can have multiple arguments, passed either as a
          tuple or a list.
...
def copySkinWeights(*args, **kwargs):
    """
    Command to copy or mirror the skinCluster weights accross one of the three major axes. The command can be used to mirror
    weights either from one surface to another or within the same surface. In query mode, return type is based on queried
    flag.
    ...
    Flags:
      ...
      - influenceAssociation : ia      (unicode)       [create,query,edit]
          ...
          influences have been matched up. Supported values are "closestJoint", "closestBone", "label", "name", "oneToOne". The
      ...
      - noBlendWeight : nbw            (bool)          [create,query,edit]
          When the no blend flag is used, the blend weights on the skin cluster will not be copied across to the destination.Flag
...
def copyKey(*args, **kwargs):
    ...
    current time unit).-time ":"is a short form to specify all keys.-index 0means the first key of each animation curve.
    (Indices are 0-based.)-index 2 -index 5 -index 7means the 3rd, 6th, and 8th keys.-index "1:5"means the 2nd, 3rd, 4th,
    5th, and 6th keys of each animation curve.This command copies curve segments's hierarchies from specified targets and
    puts them in the clipboard. Source curves are unchanged. The pasteKey command applies these curves to other objects.The
    shape of the copied curve placed in the clipboard depends on the copyKey "-option" specified. Each of these options
    below will be explained using an example. For all the explanations, let us assume that the source animation curve (from
    which keys will be copied) has 5 keyframes at times 10, 15, 20, 25, and 30. copyKey -t "12:22" -option keysA 5-frame
    animation curve with one key at 15 and another key at 20 is placed into the keyset clipboard.copyKey -t "12:22" -option
    curveA 10-frame animation is placed into the clipboard. The curve contains the original source-curve keys at times 15
    and 20, as well as new keys inserted at times 12 and 22 to preserve the shape of the curve at the given time
    ...
    Flags:
      - animLayer : al                 (unicode)       [create]
          ...
          on this layer, then no keys are copied.Flag can appear in Create mode of commandFlag can have multiple arguments, passed
          either as a tuple or a list.
      ...
      - clipboard : cb                 (unicode)       [create]
          Specifies the clipboard to which animation is copied. Valid clipboards are "api" and "anim". The default clipboard is:
          anim
      ...
      - option : o                     (unicode)       [create]
          The option to use when performing the copyKey operation. Valid options are "keys," and "curve." The default copy option
          is: keys
...
def connectJoint(*args, **kwargs):
    ...
    Flags:
      - connectMode : cm               (bool)          [create]
          The first selected joint will be parented under the parent of the second selected joint.Flag can appear in Create mode
...
def marker(*args, **kwargs):
    ...
    string argument is the parent object name.One can specify "-pm -om" option to create both, a position marker and an
    ...
    Flags:
      ...
      - detach : d                     (bool)          [create]
          This flag specifies to detach the selected position markers from their parent geometry to the 3D space.Flag can appear
          in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def simplify(*args, **kwargs):
    ...
    current time unit).-time ":"is a short form to specify all keys.-index 0means the first key of each animation curve.
    (Indices are 0-based.)-index 2 -index 5 -index 7means the 3rd, 6th, and 8th keys.-index "1:5"means the 2nd, 3rd, 4th,
    5th, and 6th keys of each animation curve.This command will simplify (reduce the number of keyframes) an animation
    curve.
    ...
    Flags:
      ...
      - floatTolerance : ft            (float)         [create]
          Specify the x-axis tolerance (defaults to 0.05) for float-input animCurves such as those created by "Set Driven
          Keyframe". This flag is ignored on animCurves driven by time. Higher floatTolerance values will result in sparser keys
          which may less accurately represent the initial curve.
      ...
      - valueTolerance : vt            (float)         [create]
          Specify the value tolerance (defaults to 0.01)Flag can appear in Create mode of commandFlag can have multiple arguments,
          passed either as a tuple or a list.
...
def cluster(*args, **kwargs):
    ...
    Flags:
      ...
      - resetGeometry : rg             (bool)          [edit]
          ...
          effects that happen if you scale an object that is deformed by a cluster.Flag can appear in Create mode of commandFlag
...
def character(*args, **kwargs):
    ...
    Flags:
      - addElement : add               (PyNode)        [edit]
          Adds the list of items to the given character. If some of the items cannot be added to the character because they are in
          another character, the command will fail. When another character is passed to to -addElement, is is added as a sub
      ...
      - isIntersecting : ii            (PyNode)        [query]
          An operation which tests whether or not the characters in the list have common members. In general, characters should be
          mutually exclusive, so this should always return false.
      ...
      - offsetNode : ofs               (bool)          [query]
          Returns the name of the characterOffset node used to add offsets to the root of the character.Flag can appear in Create
          mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
      ...
      - userAlias : ua                 (PyNode)        [query]
          ...
          Characters automatically alias the attributes where character animation data is stored. A user alias will exist when the
...
def sequenceManager(*args, **kwargs):
    """
    The sequenceManager command manages sequences, shots, and their related scenes. In query mode, return type is based on
    queried flag.
    ...
    Flags:
      ...
      - listSequencerAudio : lsa       (unicode)       [create]
          List the audio clips added to the sequencerFlag can appear in Create mode of commandFlag can have multiple arguments,
          passed either as a tuple or a list.
...
def deformer(*args, **kwargs):
    ...
    Flags:
      ...
      - type : typ                     (unicode)       [create]
          ...
          functionality.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
...
def play(*args, **kwargs):
    ...
    Flags:
      ...
      - playSound : ps                 (bool)          [create,query]
          Specify whether or not sound should be used during playbackFlag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def blendShapePanel(*args, **kwargs):
    ...
    Flags:
      - blendShapeEditor : be          (bool)          [query]
          Query only flag that returns the name of an editor to be associated with the panel.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def writeTake(*args, **kwargs):
    ...
    Flags:
      ...
      - linear : l                     (unicode)       [create]
          ...
          linear unit.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
...
def ikHandle(*args, **kwargs):
    ...
    Flags:
      ...
      - positionWeight : pw            (float)         [create,query,edit]
          ...
          and the end-effector position. A positionWeight of 1.0 computes the distance as the distance between positions only and
      ...
      - rootTwistMode : rtm            (bool)          [create,query,edit]
          ...
          remaining joints. This applies to all the twist types.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
      ...
      - solver : sol                   (unicode)       [create,query,edit]
          Specifies the solver. The complete list of available solvers may not be known until run-time because some of the solvers
          may be implemented as plug-ins. Currently the only valid solver are ikRPsolver, ikSCsolver and ikSplineSolver.
      ...
      - sticky : s                     (unicode)       [create,query,edit]
          Specifies that this handle is "sticky". Valid values are "off", "sticky", "superSticky". Sticky handles are solved when
          the skeleton is being manipulated interactively. If a character has sticky feet, the solver will attempt to keep them in
      ...
      - weight : w                     (float)         [create,query,edit]
          Specifies the handles weight in error calculations. The weight only applies when handle goals are in conflict and cannot
          be solved simultaneously. When this happens, a solution is computed that weights the "distance" from each goal to the
          solution by the handle's weight and attempts to minimize this value. The weight must be ]= 0.
...
def reroot(*args, **kwargs):
    """
    This command will reroot a skeleton. The selected joint or the given joint at the command line will be the new root of
    the skeleton. All ikHandles passing through the selected joint or above it will be deleted. The given(or selected) joint
    should not have skin attached. The command works on the given or selected joint. No options or flags are necessary.
...
def playbackOptions(*args, **kwargs):
    ...
    Flags:
      ...
      - blockingAnim : ba              (bool)          [create,query]
          All tangents playback as stepped so that animation can be viewed in pure pose-to-pose formFlag can appear in Create mode
          of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def wrinkle(*args, **kwargs):
    ...
    Flags:
      ...
      - crease : cr                    (unicode)       [create]
          Specifies an existing curve to serve as the wrinkle.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def skinPercent(*args, **kwargs):
    ...
    Flags:
      ...
      - pruneWeights : prw             (float)         [create]
          ...
          weights to zero, you must turn the -normalize flag "off" or the skinCluster node will normalize the weights to sum to
          one after pruning them. Weights for influences with a true value on their "Hold Weights" attribute will not be
          pruned.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def clipMatching(*args, **kwargs):
    ...
    Flags:
      ...
      - matchRotation : mr             (int)           [create]
          ...
          rotation 2 - Match projected rotation on ground planeFlag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
      ...
      - matchTranslation : mt          (int)           [create]
          ...
          translation 2 - Match projected translation on ground plane
...
def dopeSheetEditor(*args, **kwargs):
    ...
    Flags:
      ...
      - selectionWindow : sel          (float, float, float, float) [query,edit]
          The selection area specified as left, right, bottom, top respectively.Flag can appear in Create mode of commandFlag can
...
def movIn(*args, **kwargs):
    ...
    Flags:
      ...
      - startTime : st                 (time)          [create]
          ...
          the .mov data in the current time unit.Flag can appear in Create mode of commandFlag can have multiple arguments, passed
          either as a tuple or a list.
...
def playblast(*args, **kwargs):
    ...
    condition called "playblasting" so that users can monitor whether playblasting is occurring. You may monitor the
    condition using the API (MConditionMessage) or a script (scriptJob and condition commands).
    ...
    Flags:
      ...
      - combineSound : csd             (bool)          [create]
          ...
          match up.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
      ...
      - compression : c                (unicode)       [create]
          ...
          determine wich setting are avalible for a given platform.When the 'format' flag is 'image', this flag is used to pass in
      ...
      - format : fmt                   (unicode)       [create]
          The format of the output of this playblast. Valid values and their meanings:-- "movie" This will result in the
          defaulting movie format on a given platform On Windows, the out is AVI format and the player is Microsoft Media Player.
          On Mac OSX, the output is of type QuickTime and the player is QuickTime. On Unix, this option outputs SGI's movie format
          and sets the default viewing command to "movieplayer". The resulting file is named according to the "-f" flag, if
          specified.-- "avi" Set the format to use avi as the output format. Only supported on Windows.-- "qt" QuickTime.
          Supported on all platforms but 64 bit windows.-- "sgi" Use the older sgi format.-- "image" Outputs a sequence of image
          files and sets the viewing command to the application specified in the preferences by the user. If no application is
          specified, it sets viewing command to "fcheck". The image format will be the Output Format specified using Window-
          RenderEditors-RenderSettings-CommonTab. The resulting files use the value of the "-f" flag as a prefix, with an appended
          frame number, as in "myFile.0007.iff"-- "iff" Synonymous with "image"The default value of the -fmt/format flag is
          "movie".
      ...
      - percent : p                    (int)           [create]
          ...
          values are clamped to be within this range. A value of 25 means 1/4 of the current view size; a value of 50 means half
          the current view size; a value of 100 means full size. (Default is 50.)
      ...
      - viewer : v                     (bool)          [create]
          Specify whether a viewer should be launched for the playblast. Default is "true". Runs "fcheck" when -fmt is "image".
          The player for movie files depends on the OS: Windows uses Microsoft Media Player, Irix uses movieplayer, OSX uses
          QuickTime.
...
def setKeyframe(*args, **kwargs):
    ...
    line. The default time for the new keyframes is the current time. Override this behavior with the "-t" flag on the
    command line. The default value for the keyframe is the current value of the attribute for which a keyframe is set.
    Override this behavior with the "-v" flag on the command line. When setting keyframes on animation curves that do not
    have "time" as an input attribute (ie, they are unitless animation curves), use "-f/-float" to specify the unitless
    value at which to set a keyframe. The -time and -float flags may be combined in one command. This command sets up
    Dependency Graph relationships for proper evaluation of a given attribute at a given time.
    ...
    Flags:
      ...
      - useCurrentLockedWeights : lw   (bool)          [create]
          ...
          value. Default is falseFlag can appear in Create mode of commandFlag can have multiple arguments, passed either as a
          tuple or a list.
...
def bindSkin(*args, **kwargs):
    ...
    important.The skin is bound using the so-called "rigid" bind, in which the components are rigidly attached to the
    closest bone in the skeleton. Flexors can later be added to the skeleton to smooth out the skinning around joints.The
    skin(s) can be bound either to the entire skeleton hierarchy of the selected joint(s), or to only the selected joints.
    The entire hierarchy is the default. The -tsb/-toSelectedBones flag allows binding to only the selected bones.This
    command can also be used to detach the skin from the skeleton. Detaching the skin is useful in a variety of situations,
    such as: inserting additional bones, deleting bones, changing the bind position of the skeleton or skin, or simply
    getting rid of the skinning nodes altogether. The options to use when detaching the skin depend on how much of the
    skinning info you want to get rid of. Namely: (1) -delete or -unbind: remove all skinning nodes, (2) -unbindKeepHistory:
    remove the skinning sets, but keep the weights, (3) -disable: disable the skinning but keep the skinning sets and the
    weights.
...
def autoKeyframe(*args, **kwargs):
    ...
    on" command was issued. To stop keeping track of modified attributes, use "autoKeyframe -state off" autoKeyframe does
    not create new animation curves. An attribute must have already been keyframed (with the setKeyframe command) for
    ...
    Flags:
      - characterOption : co           (unicode)       [create,query,edit]
          Valid options are: "standard", "all". Dictates whether when auto-keying characters the auto-key works as usual or
          whether it keys all of the character attributes. Default is "standard".Flag can appear in Create mode of commandFlag can
          have multiple arguments, passed either as a tuple or a list.
...
def clipSchedule(*args, **kwargs):
    ...
    Flags:
      ...
      - absoluteRotations : abr        (bool)          [query,edit]
          This flag is now deprecated. Use aa/allAbsolute, ar/allRelative, ra/rotationsAbsolute, or da/defaultAbsolute instead. If
          true, this overrides the abs/absolute flag so that rotation channels are always calculated with absolute offsets. This
      ...
      - groupIndex : gri               (int)           [create]
          This flag specifies a multiple number of clips to be added or removed from a group.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
    
      - groupName : gn                 (unicode)       [create,query]
          This flag is used to specify the group that should be added to. If no group by that name exists and new group is created
          with that name. By default if this is not specified a new group will be created.
      ...
      - shiftIndex : shi               (int)           [create]
          ...
          shiftAmount flag. The flag specifies the index of the clip to shift. This flag can be used multiple times on the command
...
def buildKeyframeMenu(*args, **kwargs):
    """
    This command handles building the "dynamic" Keyframe menu, to show attributes of currently selected objects, filtered by
    the current manipulator. menuName is the string returned by the "menu" command. The target menu will entries appended to
    it (and deleted from it) to always show what's currently keyframable.
...
def ikSolver(*args, **kwargs):
    ...
    Flags:
      ...
      - solverType : st                (unicode)       [create,query,edit]
          valid solverType (only ikSystem knows what is valid) for creation of a new solver (required)Flag can appear in Create
          mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def bakeResults(*args, **kwargs):
    ...
    animation curves comprising a keyset depend on the value of the "-animation" flag: keysOrObjects: Any active keys, when
    no target objects or -attribute flags appear on the command line, orAll animation curves connected to all keyframable
    attributes of objects specified as the command line's targetList, when there are no active keys.keys: Only act on active
    keys or tangents. If there are no active keys or tangents, don't do anything.objects: Only act on specified objects. If
    there are no objects specified, don't do anything.Note that the "-animation" flag can be used to override the curves
    uniquely identified by the multi-use "-attribute" flag, which takes an argument of the form attributeName, such as
    "translateX". Keys on animation curves are identified by either their time values or their indices. Times and indices
    should be specified as a range, as shown below. -time "10:20"means all keys in the range from 10 to 20, inclusive, in
    the current time unit.-index "1:5"means the 2nd, 3rd, 4th, 5th, and 6th keys of each animation curve.
    ...
    Flags:
      ...
      - smart : sr                     (bool, float)   [create]
          Specify whether to enable smart bake and the optional smart bake tolerance.Flag can appear in Create mode of commandFlag
          can have multiple arguments, passed either as a tuple or a list.
...
def bufferCurve(*args, **kwargs):
    ...
    Flags:
      ...
      - exists : ex                    (bool)          [query]
          Returns true if a buffer curve currently exists on the specified attribute; false otherwise.Flag can appear in Create
          mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
      ...
      - overwrite : ov                 (bool)          [create]
          Create a buffer curve. "true" means create a buffer curve whether or not one already existed. "false" means if a buffer
...
def snapshot(*args, **kwargs):
    ...
    Flags:
      ...
      - motionTrail : mt               (bool)          [create]
          ...
          the specified time steps. Default is false.Flag can appear in Create mode of commandFlag can have multiple arguments,
          passed either as a tuple or a list.
...
def animDisplay(*args, **kwargs):
    ...
    Flags:
      - modelUpdate : upd              (unicode)       [create,query,edit]
          Controls how changes to animCurves are propagated through the dependency graph. Valid modes are "none", "interactive" or
          "delayed". If modelUpdate is "none" then changing an animCurve will not cause the model to be updated (change
          currentTime in order to update the model). If modelUpdate is "interactive" (which is the default setting), then as
          interactive changes are being made to the animCurve, the model will be updated. If modelUpdate is delayed, then the
          model is updated once the final change to an animCurve has been made. With modelUpdate set to either "interactive" or
          "delayed", changes to animCurves made via commands will also cause the model to be updated.
    
      - refAnimCurvesEditable : rae    (bool)          [create,query,edit]
          Specify if animation curves from referenced files are editable.Flag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
      ...
      - timeCodeOffset : tco           (unicode)       [create,query,edit]
          This flag has now been deprecated. It still exists to not break legacy scripts, but it will now do nothing. See the new
          timeCode command to set and query timeCodes.
...
def clip(*args, **kwargs):
    ...
    Flags:
      ...
      - absoluteRotations : abr        (bool)          [create]
          This flag is now deprecated. Use aa/allAbsolute, ar/allRelative, ra/rotationsAbsolute, or da/defaultAbsolute instead. If
          true, this overrides the -absolute flag so that rotation channels are always calculated with absolute offsets. This
          allows you to have absolute offsets on rotations and relative offsets on all other channels.
      ...
      - allClips : ac                  (bool)          [query]
          This flag is used to query all the clips in the scene. Nodes of type "animClip" that are storing poses, are not returned
          by this command.
      ...
      - allSourceClips : asc           (bool)          [query]
          This flag is used to query all the source clips in the scene. Nodes of type "animClip" that are storing poses or clip
      ...
      - mapMethod : mm                 (unicode)       [create]
          ...
          are: "byNodeName", "byAttrName", "byCharacterMap", "byAttrOrder", "byMapOrAttrName" and "byMapOrNodeName". "byAttrName"
          is the default. The flags mean the following: "byAttrOrder" maps using the order that the character stores the
          attributes internally, "byAttrName" uses the attribute name to find a correspondence, "byNodeName" uses the node name
          \*and\* the attribute name to find a correspondence, "byCharacterMap" uses the existing characterMap node to do the
          mapping. "byMapOrAttrName" uses a character map if one exists, otherwise uses the attribute name. "byMapOrNodeName" uses
          a character map if one exists, otherwise uses the attribute name.
      ...
      - rotationOffset : rof           (float, float, float) [create,query]
          Return the channel offsets used to modify the clip's rotation.Flag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def bakeClip(*args, **kwargs):
    ...
    Flags:
      ...
      - keepOriginals : k              (bool)          [create]
          ...
          clip, and to keep the original clips in the visor.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def enableDevice(*args, **kwargs):
    ...
    action-apply channelName [channelName ... ]controls if data from the device channel is applied (by default) by applyTake
    to the param curves attached to the named channel.Disabling a channel for applyTake cause applyTake to ignore the enable
    state of all "child" channels -- treating them as disabled. In query mode, return type is based on queried flag.
    
    Flags:
      - apply : a                      (bool)          [create,query]
          enable/disable "applyTake" for the specified channel(s)Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def shotRipple(*args, **kwargs):
    ...
    name is provided, the command will attempt to use the first selected shot. In query mode, return type is based on
    queried flag.
    
    Flags:
      - deleted : d                    (bool)          [create,query,edit]
          Specify whether this ripple edit is due to a shot deletionFlag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def skinCluster(*args, **kwargs):
    ...
    joints) that influence the motion of the bound skin points.The skinCluster command can also be used to adjust parameters
    such as the dropoff, nurbs samples, polygon smoothness on a particular influence object. Note: Any custom weights on a
    skin point that the influence object affects will be lost after adjusting these parameters.
    ...
    Flags:
      ...
      - dropoffRate : dr               (float)         [create,query,edit]
          ...
          is between 0.1 and 10.0. In Create mode it sets the dropoff rate for all the bound joints. In Edit mode the flag is used
          together with the inf/influence flag to set the dropoff rate of a particular influence. Note: When the flag is used in
          Edit mode, any custom weights on the skin points the given transform influences will be lost.
      ...
      - heatmapFalloff : hmf           (float)         [create]
          This flag sets the the heatmap binding falloff. If set to 0.0 (default) the deformation will be smoother due to many
          small weights spread over the mesh surface per influence. However, if set to 1.0, corresponding to maximum falloff, the
          number of influences per point will be reduced and points will tend to be greatly influenced by their closest joint
          decreasing the overall spread of small weights. This flag only works when using heatmap binding.
      ...
      - volumeType : vt                (int)           [create]
          ...
          2 - Cylinder.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
...
def findKeyframe(*args, **kwargs):
    ...
    current time unit).-time ":"is a short form to specify all keys.-index 0means the first key of each animation curve.
    (Indices are 0-based.)-index 2 -index 5 -index 7means the 3rd, 6th, and 8th keys.-index "1:5"means the 2nd, 3rd, 4th,
    5th, and 6th keys of each animation curve.This command will return the time (in current units) of the requested key. For
    the relative direction methods (next, previous) if -time is NOT specified they will use current time. If the specified
    object is not animated the command will return the current time.
    ...
    Flags:
      ...
      - curve : c                      (bool)          [create]
          ...
          and includeUpperBound flags are ignored when this flag is used.Flag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def jointDisplayScale(*args, **kwargs):
    ...
    Flags:
      ...
      - ikfk : ik                      (bool)          [create,query]
          Set the display size of ik/fk skeleton joints.Flag can appear in Create mode of commandFlag can have multiple arguments,
          passed either as a tuple or a list.
...
def pasteKey(*args, **kwargs):
    ...
    object's attributes depends on the paste "-option" specified. Each of these options below will be explained using an
    ...
    after time 5 to the right by 20 frames (to make room for the 20-frame clipboard segment).2. Paste the 20-frame clipboard
    segment starting at time 5.pasteKey -time "5:25" -option replace1. Remove all keys on the target curve from 5 to 25.2.
    Paste the 20-frame clipboard curve at time 5.pasteKey -option replaceCompletely1. Remove all keys on the target curve.2.
    Paste the 20-frame clipboard curve, preserving the clipboard curve's original keyframe times.pasteKey -time 5 -option
    merge1.The clipboard curve segment will be pasted starting at time 5 for its full 20-frame range until frame 25.2. If a
    keyframe on the target curve has the same time as a keyframe on the clipboard curve, it is overwritten. Otherwise, any
    keys that existed in the 5:25 range before the paste, will remain untouchedpasteKey -time "3:10" -option scaleInsert1.
    Shift all keyframes on the target curve after time 3 to the right by 7 frames (to clear the range 3:10 to paste in)2.
    The clipboard curve segment will be scaled to fit the specified time range (i.e. the 20 frames on the clipboard will be
    scaled to fit into 7 frames), and then pasted into the range 3:10.pasteKey -time "3:10" -option scaleReplace1. Any
    existing keyframes in the target curve in the range 3:10 are removed.2. The clipboard curve segment will be scaled to
    fit the specified time range (i.e. the 20 frames on the clipboard will be scaled to fit into 7 frames), and then pasted
    into the range 3:10.pasteKey -time "3:10" -option scaleMerge1. The clipboard curve segment will be scaled to fit the
    specified time range (i.e. the 20 frames on the clipboard will be scaled to fit into 7 frames).2. If there are keys on
    the target curve at the same time as keys on the clipboard curve, they are overwritten. Otherwise, keyframes on the
    target curve that existed in the 3:10 range before the paste, will remain untouched.pasteKey -time "3:10" -option
    fitInsert1. Shift all the keyframes on the target curve after time 3 to the right by 7 frames (to clear the range 3:10
    to paste in)2. The first 7 frames of the clipboard curve segment will be pasted into the range 3:10.pasteKey -time
    "3:10" -option fitReplace1. Any existing frames in the target curve in the range 3:10 are removed.2. The first 7 frames
    of the clipboard curve segment will be pasted into the range 3:10.pasteKey -time "3:10" -option fitMerge1. The first 7
    frames of the clipboard curve segment will be pasted into the range 3:10.2. If there are keys on the target curve at the
    same time as keys on the clipboard curve, they are overwritten. Otherwise, keyframes on the target curve that existed in
    the 3:10 range before the paste, will remain untouched.
    
    Flags:
      - animLayer : al                 (unicode)       [create]
          Specifies that the keys getting pasted should be pasted onto curves on the specified animation layer.If that layer
          doesn't exist for the specified objects or attributes then the keys won't get pasted.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
      ...
      - clipboard : cb                 (unicode)       [create]
          Specifies the clipboard from which animation is pasted. Valid clipboards are "api" and "anim". The default clipboard is:
          anim
      ...
      - option : o                     (unicode)       [create]
          Valid values are "insert", "replace", "replaceCompletely", "merge", "scaleInsert," "scaleReplace", "scaleMerge",
          "fitInsert", "fitReplace", and "fitMerge". The default paste option is: "insert".
...
def copyDeformerWeights(*args, **kwargs):
    ...
    Flags:
      ...
      - uvSpace : uv                   (unicode, unicode) [create,query,edit]
          The uvSpace flag indicates that the weight transfer should occur in UV space, based on the source and destination UV
          sets specified.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
...
def jointCluster(*args, **kwargs):
    ...
    Flags:
      ...
      - belowDropoffType : bdt         (unicode)       [create,query,edit]
          ...
          "linear", "exponential", "sine" and "none". Default is linear.
...
def deformerWeights(*args, **kwargs):
    ...
    Flags:
      - defaultValue : dv              (float)         [create,query,edit]
          ...
          If you want to force all weights to be written to disk you can set a defaultValue = -1.0.
      ...
      - method : m                     (unicode)       [create,query,edit]
          Specify the method used to map the weight during import. Valid values are: index, nearest, barycentricand over. The
          indexmethod uses the vertex index to map the weights onto the object. This is most useful when the destination object
          shares the same topology as the exported data. The nearestmethod finds the nearest vertex in the imported data set and
          sets the weight value to that value. This is best used when mapping a higher resolution mesh to a lower resolution. The
          barycentricmethod is only supported with polygon meshes. It finds the nearest triangle of the target geometry and
          rescales the weights by how close the source point is to the vertices. This is normally used with a coarse mesh being
          mapped to a high resolution mesh. The overmethod is similar to the indexmethod but the weights on the destination mesh
          are not cleared prior to mapping, so that unmatched indices keep their weights intact.
      ...
      - positionTolerance : pt         (float)         [create,query,edit]
          ...
          used with import. Defaults to a huge number.
      ...
      - worldSpace : ws                (bool)          [create,query,edit]
          For spatially based association methods (barycentric and nearest), the position should be based on the world space
          position rather then the local object space.
...
def timeWarp(*args, **kwargs):
    ...
    Flags:
      ...
      - g : g                          (bool)          [create,query,edit]
          ...
          the global time warp node. Note: only one global time warp can exist in the scene.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
      ...
      - moveFrame : mf                 (int, float)    [query,edit]
          ...
          When queried, it returns an array of the warped frame values.
...
def ikSystemInfo(*args, **kwargs):
    ...
    Flags:
      - globalSnapHandle : gsh         (bool)          [create,query]
          If this flag is off, all ikHandles will not be snapped.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def jointLattice(*args, **kwargs):
    ...
    Flags:
      ...
      - lowerBindSkin : lb             (unicode)       [create]
          Specifies the node which is performing the bind skin operation on the geometry associated with the lower bone.Flag can
...
def bakeSimulation(*args, **kwargs):
    ...
    current time unit).-time ":"is a short form to specify all keys.-index 0means the first key of each animation curve.
    (Indices are 0-based.)-index 2 -index 5 -index 7means the 3rd, 6th, and 8th keys.-index "1:5"means the 2nd, 3rd, 4th,
    5th, and 6th keys of each animation curve.The bakeSimulation command is obsolete. Instead, "bakeResults -simulation
    true" should be used. The bakeSimulation command has retained for backwards compatibility. This command allows the user
    to replace a chain of dependency nodes, or implicit relationship, such as those between joints and IK handles, which
    define the value of an attribute, with a single animation curve. Command allows the user to specify the range and
    frequency of sampling. Unlike the bakeResults command, this command will actually set the time of the current scene to
    all the times, in sequence, inside the given time interval before it sets the time back to when it is started. As a
    result, it may modify the scene. In query mode, return type is based on queried flag.
    ...
    Flags:
      ...
      - preserveOutsideKeys : pok      (bool)          [create]
          Whether to preserve keys that are outside the bake range when there are directly connected animation curves. The default
          (false) is to remove frames outside the bake range. If the channel that you are baking is not controlled by a single
          animation curve, then a new animation curve will be created with keys only in the bake range.
    
      - removeBakedAttributeFromLayer : ral (bool)          [create]
          If true, all baked attributes will be removed from the layer. Otherwise all layers associated with the baked attribute
          will be muted.
      ...
      - sparseAnimCurveBake : sac      (bool)          [create]
          When baking anim curves, do not insert any keys into areas of the curve where animation is defined. And, use as few keys
          as possible to bake the pre and post infinity behaviors. When this is false, one key will be inserted at each time step.
...
def keyframeStats(*args, **kwargs):
    """
    All of the group commands position their individual controls in columns starting at column 1. The layout of each control
    (ie. column) can be customized using the -cw/columnWidth, -co/columnOffset, -cat/columnAttach, -cal/columnAlign, and
    -adj/adjustableColumnflags. By default, columns are left aligned with no offset and are 100 pixels wide. Only one column
    in any group can be adjustable. This command creates, edits, queries a keyframe stats control.
    ...
    Flags:
      ...
      - columnAlign2 : cl2             (unicode, unicode) [create,edit]
          Sets the text alignment of both columns. Ignored if there is not exactly two columns. Valid values are "left", "right",
          and "center".
    
      - columnAlign3 : cl3             (unicode, unicode, unicode) [create,edit]
          Sets the text alignment for all 3 columns. Ignored if there is not exactly 3 columns. Valid values are "left", "right",
          and "center".
    
      - columnAlign4 : cl4             (unicode, unicode, unicode, unicode) [create,edit]
          Sets the text alignment for all 4 columns. Ignored if there is not exactly 4 columns. Valid values are "left", "right",
          and "center".
    
      - columnAlign5 : cl5             (unicode, unicode, unicode, unicode, unicode) [create,edit]
          Sets the text alignment for all 5 columns. Ignored if there is not exactly 5 columns. Valid values are "left", "right",
          and "center".
    
      - columnAlign6 : cl6             (unicode, unicode, unicode, unicode, unicode, unicode) [create,edit]
          Sets the text alignment for all 6 columns. Ignored if there is not exactly 6 columns. Valid values are "left", "right",
          and "center".
      ...
      - valueAnnotation : van          (unicode)       [create,query,edit]
          Annotate the value field with an extra string value.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def ikfkDisplayMethod(*args, **kwargs):
    """
    The ikfkDisplayMethodcommand is used to specify how ik/fk blending should be shown In query mode, return type is based
    on queried flag.
    ...
    Flags:
      - display : d                    (unicode)       [create,query]
          ...
          blending), "ik" (only show ik),"fk" (only show fk), and "ikfk" (show both ik and fk).Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def cutKey(*args, **kwargs):
    ...
    current time unit).-time ":"is a short form to specify all keys.-index 0means the first key of each animation curve.
    (Indices are 0-based.)-index 2 -index 5 -index 7means the 3rd, 6th, and 8th keys.-index "1:5"means the 2nd, 3rd, 4th,
    5th, and 6th keys of each animation curve.The cutKey command cuts curve segment hierarchies from specified targets and
    puts them in the clipboard. The pasteKey command applies these curves to other objects.The shape of the cut curve placed
    in the clipboard, and the effect of the cutKey command on the source animation curve depends on the cutKey "-option"
    specified. Each of these options below will be explained using an example. For all the explanations, let us assume that
    the source animation curve (from which keys will be cut) has 5 keyframes at times 10, 15, 20, 25, and
    30.TbaseKeySetCmd.h cutKey -t "12:22" -option keysKeyframes at times 15 and 20 are removed. All other keys are
    unchanged.A 5-frame animation curve is placed into the keyset clipboard.cutKey -t "12:22" -option keysCollapseKeyframes
    at times 15 and 20 are removed. Shift all keys after time 20 to the left by 5 frames, preserving all their values.A
    5-frame animation curve is placed into the keyset clipboard.cutKey -t "12:22" -option keysConnectKeyframes at times 15
    and 20 are removed. Shift all keys after time 20 to the left by 5 frames, and place the key that used to be at time 25
    at the value of the key that used to be at time 15.A 5-frame animation curve is placed into the keyset clipboard.cutKey
    -t "12:22" -option curveKeyframes at times 15 and 20 are removed. Keys are inserted at times 12 and 22.A 10-frame
    animation curve is placed into the keyset clipboard.cutKey -t "12:22" -option curveCollapseKeyframes at times 15 and 20
    are removed. Keys are inserted at times 12 and 22. Shift all keys from time 22 to the left by 10 frames, preserving
    their values.A 10-frame animation curve is placed into the keyset clipboard.cutKey -t "12:22" -option
    curveConnectKeyframes at times 15 and 20 are removed. Keys are inserted at times 12 and 22. Shift all keys from time 22
    to the left by 10 frames, and replace the key inserted at time 12 with the newly inserted key at time 22.A 10-frame
    animation curve is placed into the keyset clipboard.cutKey -t "12:22" -option areaCollapseKeyframes at times 15 and 20
    are removed. Shift all keys from time 22 to the left by 10 frames, preserving their values.A 10-frame animation curve is
    placed into the keyset clipboard.
    ...
    Flags:
      ...
      - selectKey : sl                 (bool)          [create]
          Select the keyframes of curves which have had keys removedFlag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def movOut(*args, **kwargs):
    ...
    Flags:
      ...
      - precision : pre                (int)           [create]
          Sets the number of digits to the right of the decimal place in the .mov file.C: The default is 6.Flag can appear in
          Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def defineDataServer(*args, **kwargs):
    ...
    the device is defined, it queries queries the server for data axis information. The "CapChannels" present are
    represented as axis in form "channelName"."usage" for scalar channels and "channelName"."component" for compound
    channels. See listInputDeviceAxesto list axis names. Note that undoing defineDataServer -d "myDevice" -s "myServer" does
    not break the connection with the data server until it cannot be redone. Executing any other command (sphere for
    example) will cause this to occur. Similarly, the command defineDataServer -d "myDevice" -u does not break the
    connection with the data server until it cannot be undone. Either flushUndo, or the 'defineDataServer' command falling"
    off the end of the undo queue causes this to occur, and the connection. to be broken. No return value.
    ...
    Flags:
      ...
      - server : s                     (unicode)       [create]
          ...
          with "/", namedenotes the full path to the socket.host:service- a udp service on the specified host.The servicecan be
          any one of a "udp service name," a "port number," or a named service of "tcpmux," and they are found in that order. If
          hostis omitted, the localhost is used.In any case, if the server cannot be found, the device is not defined (created)
          and the command fails.
    
      - undefine : u                   (bool)          [create]
          undefines (destroys) the dataServer device, closing the connection with the server.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def setInfinity(*args, **kwargs):
    ...
    Flags:
      ...
      - postInfinite : poi             (unicode)       [create,query]
          Set the infinity type after a paramCurve's last keyframe. Valid values are "constant", "linear", "cycle",
          "cycleRelative", "oscillate".Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as
          a tuple or a list.
    
      - preInfinite : pri              (unicode)       [create,query]
          Set the infinity type before a paramCurve's first keyframe. Valid values are "constant", "linear", "cycle",
...
def keyframe(*args, **kwargs):
    ...
    current time unit).-time ":"is a short form to specify all keys.-index 0means the first key of each animation curve.
    (Indices are 0-based.)-index 2 -index 5 -index 7means the 3rd, 6th, and 8th keys.-index "1:5"means the 2nd, 3rd, 4th,
    5th, and 6th keys of each animation curve.This command edits the time and/or value of keys of specified objects and/or
    parameter curves Unless otherwise specified by the -query flag, the command defaults to editing keyframes.
    ...
    Flags:
      ...
      - breakdown : bd                 (bool)          [create,query,edit]
          Sets the breakdown state for the key. Returns an integer. Default is false. The breakdown state of a key cannot be set
          in the same command as it is moved (i.e., via the -tc or -fc flags).
      ...
      - eval : ev                      (bool)          [create,query]
          ...
          -t/time or -f/float flags. Cannot be used in combination with other query flags, and cannot be used with time ranges (-t
          "5:10"). When no -t or -f flags appear on the command line, the evals are queried at the current time. Query returns a
          float[].
      ...
      - lastSelected : lsl             (bool)          [create,query]
          When used in queries, this flag returns requested values for the last selected key.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def flow(*args, **kwargs):
    ...
    Flags:
      ...
      - objectCentered : oc            (bool)          [query]
          ...
          around the curve.Default value is true.When queried, it returns a booleanFlag can appear in Create mode of commandFlag
          can have multiple arguments, passed either as a tuple or a list.
...
def sculpt(*args, **kwargs):
    ...
    Flags:
      ...
      - sculptTool : st                (unicode)       [create]
          Use the specified NURBS object as the sculpt tool instead of the default implicit sphere.Flag can appear in Create mode
          of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def keyTangent(*args, **kwargs):
    ...
    current time unit).-time ":"is a short form to specify all keys.-index 0means the first key of each animation curve.
    (Indices are 0-based.)-index 2 -index 5 -index 7means the 3rd, 6th, and 8th keys.-index "1:5"means the 2nd, 3rd, 4th,
    5th, and 6th keys of each animation curve.This command edits or queries tangent properties of keyframes in a keyset. It
    is also used to edit or query the default tangent type of newly created keyframes (see the setKeyframe command for more
    information on how to override this default). Tangents help manage the shape of the animation curve and affect the
    interpolation between keys. The tangent angle specifies the direction the curve will take as it leaves (or enters) a
    key. The tangent weight specifies how much influence the tangent angle has on the curve before the curve starts towards
    the next key. Maya internally represents tangents as x and y values. Refer to the API documentation for MFnAnimCurve for
    a description of the relationship between tangent angle and weight and the internal x and y values.
    ...
    Flags:
      ...
      - inTangentType : itt            (unicode)       [create,query,edit]
          Specify the in-tangent type. Valid values are "spline," "linear," "fast," "slow," "flat," "step," "stepnext," "fixed,"
          "clamped," "plateau" and "auto". Returns a string[] when queried.
      ...
      - outTangentType : ott           (unicode)       [create,query,edit]
          Specify the out-tangent type. Valid values are "spline," "linear," "fast," "slow," "flat," "step," "stepnext," "fixed,"
          "clamped," "plateau" and "auto". Returns a string[] when queried.
      ...
      - stepAttributes : sa            (bool)          [create,edit]
          The setKeyframe command will automatically set tangents for boolean and enumerated attributes to step. This flag mirrors
          this behavior for the keyTangent command. When set to false, tangents for these attributes will not be edited. When set
          to true (the default) tangents for these attributes will be edited.Flag can appear in Create mode of commandFlag can
          have multiple arguments, passed either as a tuple or a list.
...
def characterize(*args, **kwargs):
    ...
    Flags:
      ...
      - addMissingEffectors : ame      (bool)          [edit]
          ...
          the user has deleted some effectors or added fingers to an existing skeleton, "characterize -e -addMissingEffectors" can
      ...
      - effectors : ef                 (unicode)       [create]
          Specify the effectors to be used by human IK by providing 2 pieces of information for each effector: 1) the partial path
          of the effector and 2) the name of the full body effector this represents. 1) and 2) are to be separated by white space,
          and multiple entries separated by ",". Normally, the effectors are automatically created. This flag is for advanced
          users only.
      ...
      - name : nm                      (unicode)       [create]
          ...
          supported.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
      ...
      - type : typ                     (unicode)       [create]
          Specifies the technique used by the characterization to identify the joint type. Valid options are "label" and "name".
          When "label" is used, the joints must be labelled using the guidelines described in the Maya documentation. When name is
          used, the joint names must follow the naming conventions described in the Maya documentation. The default is "name".
          This flag cannot be used in conjunction with the sourceSkeleton flag.
...
def characterMap(*args, **kwargs):
    ...
    Flags:
      ...
      - mapMethod : mm                 (unicode)       [create]
          ...
          "byAttrName", and "byAttrOrder". "byAttrOrder" is the default. The flags mean the following: "byAttrOrder" maps using
          the order that the character stores the attributes internally, "byAttrName" uses the attribute name to find a
          correspondence, "byNodeName" uses the node name \*and\* the attribute name to find a correspondence.
      ...
      - unmapNode : umn                (unicode, unicode) [create]
          ...
          node. Note that mapped attributes which do not have matching names, will not be unmapped.Flag can appear in Create mode
          of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def volumeBind(*args, **kwargs):
    ...
    Flags:
      - influence : inf                (bool)          [query,edit]
          Edit or Query the list of influences connected to the skin cluster.Flag can appear in Create mode of commandFlag can
          have multiple arguments, passed either as a tuple or a list.
...
def parentConstraint(*args, **kwargs):
    ...
    target's contribution to the position and rotation of the object. A parentConstraint takes as input one or more "target"
    DAG transform nodes at which to position and rotate the single "constraint object" DAG transform node. The
    ...
    Flags:
      ...
      - skipTranslate : st             (unicode)       [create]
          Causes the axis specified not to be considered when constraining translation. Valid arguments are "x", "y", "z" and
          "none".
...
def aimConstraint(*args, **kwargs):
    ...
    aimConstraint takes as input one or more "target" DAG transform nodes at which to aim the single "constraint object" DAG
    transform node. The aimConstraint orients the constrained object such that the aimVector (in the object's local
    coordinate system) points to the in weighted average of the world space position target objects. The upVector (again the
    the object's local coordinate system) is aligned in world space with the worldUpVector.
    ...
    Flags:
      ...
      - skip : sk                      (unicode)       [create,edit]
          Specify the axis to be skipped. Valid values are "x", "y", "z" and "none". During creation, "none" is the default.Flag
          can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def animLayer(*args, **kwargs):
    ...
    Flags:
      ...
      - findCurveForPlug : fcv         (unicode)       [edit]
          finds the parameter curve containing the animation data for the specified plug on the given layer.Flag can appear in
          Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def dagPose(*args, **kwargs):
    ...
    Flags:
      ...
      - worldParent : wp               (bool)          [create]
          ...
          when you plan to reparent the object to world as the next operation.Flag can appear in Create mode of commandFlag can
          have multiple arguments, passed either as a tuple or a list.
...
def substituteGeometry(*args, **kwargs):
    ...
    Flags:
      ...
      - reWeightDistTolerance : wdt    (float)         [create]
          ...
          algorithm to find more corresponding vertices.Flag can appear in Create mode of commandFlag can have multiple arguments,
          passed either as a tuple or a list.
...
def snapKey(*args, **kwargs):
    ...
    current time unit).-time ":"is a short form to specify all keys.-index 0means the first key of each animation curve.
    (Indices are 0-based.)-index 2 -index 5 -index 7means the 3rd, 6th, and 8th keys.-index "1:5"means the 2nd, 3rd, 4th,
    5th, and 6th keys of each animation curve.This command "snaps" all target key times and/or values so that they have
    times and/or values that are multiples of the specified flag arguments. If neither multiple is specified, default is
    time snapping with a multiple of 1.0. Note that this command will fail to move keys over other neighboring keys: a key's
    index will not change as a result of a snapKey operation.TbaseKeySetCmd.h
    ...
    Flags:
      ...
      - valueMultiple : vm             (float)         [create]
          If this flag is present, key values will be snapped to a multiple of the specified float value.Flag can appear in Create
          mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def softMod(*args, **kwargs):
    ...
    Flags:
      ...
      - curveInterpolation : ci        (int)           [create]
          ...
          "none", "linear", "smooth" and "spline" respectively. This flag may only be used in conjunction with the curvePoint and
          curveValue flag.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
...
def deviceManager(*args, **kwargs):
    """
    This command queriers the internal device manager for information on attached devices. In query mode, return type is
    based on queried flag.
    ...
    Flags:
      ...
      - axisCoordChanges : acc         (bool)          [query]
          Returns whether the axis coordinate changes. Expects the -deviceIndex and -axisIndex flags to be used in
          conjunction.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
...
def movieInfo(*args, **kwargs):
    ...
    Flags:
      ...
      - movieTexture : mt              (bool)          [create]
          ...
          movie loaded by that node.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a
          tuple or a list.
...
def effector(*args, **kwargs):
    ...
    Flags:
      - hide : hi                      (bool)          [create,query,edit]
          Specifies whether to hide drawing of effector if attached to a handle.Flag can appear in Create mode of commandFlag can
          have multiple arguments, passed either as a tuple or a list.
...
def filterCurve(*args, **kwargs):
    """
    The filterCurve command takes a list of anim curve and filters them. Currently only a Euler filter is supported. The
    Euler filter demangles discontinous rotation anim curves into smooth curves.
    ...
    Flags:
      - endTime : e                    (time)          [create]
          ...
          the end time.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
      ...
      - kernel : ker                   (unicode)       [create]
          ...
          moving averagetriangleTriangle filter: (1 - |x|) weighted moving averagegaussian2Gaussian2 Filter: (2^(-2x\*x)) weighted
          moving averagegaussian4Gaussian4 Filter: (2^(-4x\*x)) weighted moving averageThis filter is onlytargeted at decimation
          resampling -- interpolation resampling is basically unsupported. If your output framerate is much higher than your input
          frame rate (approximate, as the input timestep is not assumed to be regular) the lirp and triangle will interpolate
          (usually) and the rest will either average, or use the closest sample (depending on the phase and frequency of the
          input). However this mode of operation may not give the expected result.
...
def sound(*args, **kwargs):
    ...
    Flags:
      ...
      - mute : m                       (bool)          [create,query,edit]
          Mute the audio clip.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple
          or a list.
...
def choice(*args, **kwargs):
    ...
    Flags:
      ...
      - controlPoints : cp             (bool)          [create]
          Explicitly specify whether or not to include the control points of a shape (see "-s" flag) in the list of attributes.
          Default: false.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
...
def pointConstraint(*args, **kwargs):
    """
    Constrain an object's position to the position of the target object or to the average position of a number of targets. A
    pointConstraint takes as input one or more "target" DAG transform nodes at which to position the single "constraint
    object" DAG transform node. The pointConstraint positions the constrained object at the weighted average of the world
...
def buildBookmarkMenu(*args, **kwargs):
    """
    This command handles building the "dynamic" Bookmark menu, to show all bookmarks ("sets") of a specified type ("sets
    -text") menuName is the string returned by the "menu" command.
    
    Flags:
      - editor : ed                    (unicode)       [create]
          Name of the editor which this menu belongs toFlag can appear in Create mode of commandFlag can have multiple arguments,
          passed either as a tuple or a list.
...
def ikSystem(*args, **kwargs):
    ...
    Flags:
      - allowRotation : ar             (bool)          [query,edit]
          Set true to allow rotation of an ik handle with keys set on translation.Flag can appear in Create mode of commandFlag
          can have multiple arguments, passed either as a tuple or a list.
...
def mute(*args, **kwargs):
    ...
    Flags:
      ...
      - force : f                      (bool)          [create]
          ...
          both be removed. If this flag is not set, then mute nodes with animation will only be disabled.Flag can appear in Create
          mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def currentTime(*args, **kwargs):
    """
    When given a time argument (with or without the -edit flag) this command sets the current global time. The model updates
    and displays at the new time, unless "-update off" is present on the command line.
    ...
    Flags:
      - update : u                     (bool)          [create]
          change the current time, but do not update the world. Default value is true.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def blendTwoAttr(*args, **kwargs):
    ...
    Flags:
      ...
      - time : t                       (timerange)     [create]
          ...
          blend will cause an abrupt change from the first to the second attribute at that time. If a range is specified, a smooth
...
def rotationInterpolation(*args, **kwargs):
    """
    The rotationInterpolation command converts the rotation curves to the desired rotation interpolation representation. For
    example, an Euler-angled representation can be converted to Quaternion. In query mode, return type is based on queried
    flag.
    ...
    Flags:
      - convert : c                    (unicode)       [create,query]
          ...
          individual tangents), "quaternionSlerp" (applies quaternion slerp interpolation to the curve, ignoring tangent
          settings), "quaternionSquad" (applied cubic interpolation to the curve in quaternion space, ignoring tangent
          settings)Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def blendShape(*args, **kwargs):
    ...
    Flags:
      ...
      - normalizationGroups : ng       (bool)          [query]
          Returns a list of the used normalization group IDs.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
    
      - origin : o                     (unicode)       [create]
          blendShape will be performed with respect to the world by default. Valid values are "world" and "local". The local flag
          will cause the blend shape to be performed with respect to the shape's local origin.
...
def lattice(*args, **kwargs):
    ...
    Flags:
      ...
      - dualBase : db                  (bool)          [create]
          ...
          creation of a normal ffd deformer node. Intended for internal usage only.Flag can appear in Create mode of commandFlag
          can have multiple arguments, passed either as a tuple or a list.
      ...
      - outsideLattice : ol            (int)           [create]
          ...
          points which fall within the "falloff distance" (see the -ofd/outsideFalloffDistance flag) are transformed. When
...
def recordDevice(*args, **kwargs):
    ...
    Flags:
      - cleanup : c                    (bool)          [create]
          Removes the recorded data from the device.Flag can appear in Create mode of commandFlag can have multiple arguments,
          passed either as a tuple or a list.
    
      - data : da                      (bool)          [query]
          Specifies if the device has recorded data. If the device is recording at the time of query, the flag will return
          false.Q: When queried, this flag returns an int.
      ...
      - playback : p                   (bool)          [create,query]
          ...
          including any animation curves attached to attributes being recorded.C: The default is false.Q: When queried, this flag
          returns an int.
...
def clipEditor(*args, **kwargs):
    ...
    Flags:
      ...
      - manageSequencer : ms           (bool)          [create,query,edit]
          ...
          represented.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
    
      - menuContext : mc               (unicode)       [query]
          Returns a string array denoting the type of data object the cursor is over. Returned values are: {"timeSlider"}
          {"nothing"} {"track", "track index", "character node name", "group name"} {"clip", "clip node name"
