...
def textureWindow(*args, **kwargs):
    """
    This command is used to create a UV Texture Editor and to query or edit the texture editor settings. The UV Texture
    Editor displays texture mapped polygon objects in 2D texture space. Only active objects are visible in this window. The
    UV Texture Editor has the ability to display two types of images. The Texture Image is a visualisation of the current
    texture and associated placement parameters. The Editor Image is a user specified image loaded from disk. A UV Texture
    Editor can be invoked by selecting the "Window -UV Texture Editor..." menu item from the main maya menu listing that
    appears at the top of the maya window. It can also be invoked by selecting the "Panel -UV Texture Editor" item under the
    "Panels" menu item that appears at the top right hand corner of the view. As a UV Texture Editor typically exists at
    start-up time, and as only one of these can exist at any given time, this command is normally used to query and edit the
    editor settings.
    ...
    Flags:
      ...
      - displaySolidMap : dsm          (bool)          [create,query,edit]
          Display an solid over lay for the active texture map.Flag can appear in Create mode of commandFlag can have multiple
      ...
      - imageRatio : imr               (bool)          [query,edit]
          ...
          than than the width is set to be 1 "unit" in the window and the height is adjusted by width divided by height. This only
      ...
      - imageUnfiltered : iuf          (bool)          [query,edit]
          Sets the Texture Image to draw unfiltered. The image will appear "pixelated" when the display resolution is higher than
...
def nodePreset(*args, **kwargs):
    ...
    Flags:
      - attributes : atr               (unicode)       [create]
          ...
          be stored.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
      ...
      - save : sv                      (PyNode, <type 'unicode'>) [create]
          ...
          the value of the attribute will not be written as part of the preset.
...
def lsThroughFilter(*args, **kwargs):
    ...
    Flags:
      ...
      - reverse : rv                   (bool)          [create]
          Only available in conjunction with nodeArray flag. Reverses the order of nodes in the returned arrays if true.Flag can
      ...
      - sort : so                      (unicode)       [create]
          Only available in conjunction with nodeArray flag. Orders the nodes in the returned array. Current options are:
...
def spotLight(*args, **kwargs):
    ...
    Flags:
      ...
      - shadowDither : sd              (float)         [create,query,edit]
          dither the shadowFlag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or
          a list.
...
def renderPassRegistry(*args, **kwargs):
    ...
    Flags:
      ...
      - isPassSupported : ips          (bool)          [create]
          ...
          renderer whose default value is the current renderer is specified by the flag renderer.Flag can appear in Create mode of
...
def camera(*args, **kwargs):
    ...
    Flags:
      ...
      - filmFit : ff                   (unicode)       [create,query,edit]
          ...
          arbitrary (and different) resolution, relating the two can get complicated. There are 4 choices: horizontalIn this case
      ...
      - focalLength : fl               (float)         [create,query,edit]
          This is the distance along the lens axis between the lens and the film plane when "focal distance" is infinitely large.
      ...
      - homeCommand : hc               (unicode)       [create,query,edit]
          Specify the command to execute when "viewSet -home" is applied to this camera. All occurances of "%camera" will be
          replaced with the cameras name before viewSet runs the command.
      ...
      - horizontalFieldOfView : hfv    (float)         [create,query,edit]
          ...
          convenience for the user. Its derivation is aov = 2 \* atan( fbw / (2 \* f) ) where "aov" is the angle of view, "fbw" is
          the film back width and "f" is the focal length.
      ...
      - lensSqueezeRatio : lsr         (float)         [create,query,edit]
          ...
          fact that a "net" distortion parameter could be used for both may or may not confuse the issue.
      ...
      - worldUp : wup                  (float, float, float) [create,query,edit]
          Camera world up vector.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a
          tuple or a list.
...
def checkDefaultRenderGlobals(*args, **kwargs):
    ...
    force the scene to be dirty/clean use `file -modified` In query mode, return type is based on queried flag.
...
def iprEngine(*args, **kwargs):
    ...
    Flags:
      - copy : cp                      (unicode)       [edit]
          Copies the deep raster file, as well as its related files, to the new location.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
      ...
      - scanlineIncrement : sli        (unicode)       [create,query,edit]
          Set the scanline increment percentage. If the height of the region being update is 240 pixels, and the scanlineIncrement
...
def renderWindowEditor(*args, **kwargs):
    ...
    Flags:
      ...
      - colorManage : com              (bool)          [edit]
          ...
          manager attached to the view.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as
          a tuple or a list.
    
      - compDisplay : cd               (int)           [create,query,edit]
          0 : disable compositing.1 : displays the composited image immediately.For example, when foreground layer tile is sent to
          the render view window, the composited tile is displayed in the render view window, and the original foreground layer
      ...
      - displayImageViewCount : dvc    (int)           [query]
          ...
          "viewImageCount" which returns the number of views for the current rendered image.
      ...
      - stereo : s                     (int)           [create,query,edit]
          Puts the editor into stereo image mode. The effective resolution of the output image is twice the size of the horizontal
          size. The orientation of the images can be set using the stereoOrientation flag.
    
      - stereoImageOrientation : sio   (unicode, unicode) [create,query,edit]
          Specifies the orientation of stereo camera renders. The first argument specifies the orientation value for the firstleft
          image and the second argument specifies the orientation value for the right image. The orientation values are 'normal',
          the image appears as seen throught he camera, or 'mirrored', the image is mirrored horizontally.
...
def renderInfo(*args, **kwargs):
    ...
    Flags:
      ...
      - vtype : vt                     (int)           [create]
          Tessellation subdivision criteria.Flag can appear in Create mode of commandFlag can have multiple arguments, passed
...
def displacementToPoly(*args, **kwargs):
    ...
    Flags:
      - findBboxOnly : fbb             (bool)          [create,query,edit]
          When used, only the bounding box scale for the displaced object is found.Flag can appear in Create mode of commandFlag
...
def convertSolidTx(*args, **kwargs):
    ...
    Flags:
      ...
      - backgroundMode : bm            (unicode)       [create]
          Defines how the background of the texture should be filled. Three modes are available:"shader"or 1: uses the default
          shader color."color"or 2: uses the color given by -backgroundColor flag."extend"or 3: extends outwards the color along
          the seam edges.Default is "shader".
      ...
      - pixelFormat : pf               (unicode)       [create]
          Specifies the pixel format of the image. Note that not all file formats support all pixel formats. Available
          options:"8": 8 bits per channel, unsigned (0-255)"16": 16 bits per channel, unsigned (0-65535)Default is "8".Flag can
          appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def panZoom(*args, **kwargs):
    ...
    Flags:
      ...
      - zoomRatio : z                  (float)         [create]
          Set the amount of zoom ratioFlag can appear in Create mode of commandFlag can have multiple arguments, passed either as
          a tuple or a list.
...
def viewPlace(*args, **kwargs):
    ...
    Flags:
      ...
      - upDirection : up               (float, float, float) [create]
          The new up direction vector.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as
          a tuple or a list.
...
def ogsRender(*args, **kwargs):
    ...
    Flags:
      ...
      - activeRenderTargetFormat : fpt (unicode)       [query,edit]
          Query the current active floating point target format.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def shadingNode(*args, **kwargs):
    """
    The shadingNode command classifies any DG node as a shader, texture light, post process, or utility so that it can be
    properly organized in the multi-lister. Recall that any DG node can be used a part of a a shader, texture or light -
    regardless of how it is classified by this. command. These classifications are provided for convenience in the UI.
    ...
    Flags:
      ...
      - asRendering : ar               (bool)          [create]
          classify the current DG node as a rendering nodeFlag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def renderer(*args, **kwargs):
    ...
    command also allow you to query the UI name and the procedure names for the registered renders. In query mode, return
    ...
    Flags:
      ...
      - globalsNodes : gn              (bool)          [create,query,edit]
          This flag returns the list of render globals nodes the specified renderer uses.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
    
      - globalsTabCreateProcNames : gtc (bool)          [create,query,edit]
          This flag returns the names of procedures which are used to create the unified render globals window tabs which are
      ...
      - globalsTabUpdateProcNames : gtu (bool)          [create,query,edit]
          This flag returns the names of procedures which are used to update the unified render globals window tabs which are
          associated with the specified renderer.
      ...
      - logoCallbackProcedure : lgc    (unicode)       [create,query,edit]
          Set or query the procedure which is a callback associated to the logo for the specified renderer. For example, the logo
          and the callback can be used in the unified render globals window beside the "Render Using" optionMenu.
      ...
      - renderingEditorsSubMenuProcedure : res (unicode)       [create,query,edit]
          Set or query the procedure reponsible for creating renderer specific editors submenu under the "Rendering Editors" menu
...
def tumble(*args, **kwargs):
    ...
    (<function tumble at 0x1493346e0>, <function addCmdDocsCallback at 0x1490edb18>, ('tumble', ''), {})
    ...
    Flags:
      ...
      - localTumble : lt               (int)           [create]
          ...
          interest, and 2 for the camera's local axis, offset by its tumble pivot.Flag can appear in Create mode of commandFlag
          can have multiple arguments, passed either as a tuple or a list.
...
def cameraSet(*args, **kwargs):
    ...
    Flags:
      ...
      - clearDepth : cd                (bool)          [create,query,edit]
          Specifies if the drawing buffer should be cleared before beginning the draw for that layer.Flag can appear in Create
...
def lookThru(*args, **kwargs):
    ...
    Flags:
      - farClip : fc                   (float)         [create]
          ...
          values must come before shape or view.Flag can appear in Create mode of commandFlag can have multiple arguments, passed
          either as a tuple or a list.
...
def binMembership(*args, **kwargs):
    ...
    Flags:
      ...
      - isValidBinName : ivn           (unicode)       [create]
          Query if the specified bin name is valid. If so, return true. Otherwise, return false.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def viewCamera(*args, **kwargs):
    ...
    Flags:
      ...
      - sideView : s                   (bool)          [create]
          Position camera to look at the side of the target camera.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def spotLightPreviewPort(*args, **kwargs):
    ...
    Flags:
      ...
      - widthHeight : wh               (int, int)      [create]
          The width and height of the port.Flag can appear in Create mode of commandFlag can have multiple arguments, passed
          either as a tuple or a list.
...
def createRenderLayer(*args, **kwargs):
    ...
    Flags:
      - empty : e                      (bool)          [create]
          If set then create an empty render layer. The global flag or specified member list will take precidence over use of this
          flag.
      ...
      - number : num                   (int)           [create]
          Number for the new render layer being created.Flag can appear in Create mode of commandFlag can have multiple arguments,
...
def viewClipPlane(*args, **kwargs):
    ...
    Flags:
      ...
      - farClipPlane : fcp             (float)         [create,query]
          Set or query the far clip plane.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either
...
def hwReflectionMap(*args, **kwargs):
    ...
    Flags:
      ...
      - sphereMapTextureName : smn     (unicode)       [query]
          ...
          string.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def viewFit(*args, **kwargs):
    ...
    Flags:
      ...
      - animate : an                   (bool)          [create]
          Specifies that the transition between camera positions should be animated.Flag can appear in Create mode of commandFlag
          can have multiple arguments, passed either as a tuple or a list.
...
def editRenderLayerMembers(*args, **kwargs):
    ...
    Flags:
      ...
      - remove : r                     (bool)          [create]
          Remove the specified objects from the render layer.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def dolly(*args, **kwargs):
    ...
    Flags:
      ...
      - dollyTowardsCenter : dtc       (bool)          [create]
          ...
          the user clicks (if false). By default, dollyTowardsCenter is on.Flag can appear in Create mode of commandFlag can have
...
def hwRender(*args, **kwargs):
    ...
    Flags:
      ...
      - activeTextureCount : atc       (bool)          [query]
          This flag when used with query will return the number of textures that have been bound to the graphics by the hardware
          renderer.
      ...
      - fixFileNameNumberPattern : fnp (bool)          [create,query]
          This flag allows the user to take the hardwareRenderGlobals filename as the initial filename pattern, fix the frame
          number pattern in the filename in a unique way, returns the new filename pattern. This does not change the
      ...
      - fullRenderSupport : frs        (bool)          [create,query]
          This flag may be used in the create or query context. In the create context, it will force the renderer to abort and not
          render any frames if the hardware is not fully supported. In the query context, it will return whether full quality
          rendering is supported on the current graphics system. Please see the graphics card qualification charts for an
          explanation of limited support.
    
      - height : h                     (int)           [create,query]
          The height flag pass the height to the hwRender command. If not used, the height is taken from the render globals
          settings.
    
      - imageFileName : ifn            (bool)          [create,query]
          This flag let people query the image name for a specified frame. The frame can be specified using the "-frame" flag.
          When no "-frame" is used, the current frame number is used.
    
      - layer : l                      (PyNode)        [create,query]
          Render the specified render layer. Only this render layer will be rendered, regardless of the renderable attribute value
          of the render layer. The layer name will be appended to the output image file name. The specified render layer becomes
          the current render layer before rendering, and remains as current render layer after the rendering.Flag can appear in
      ...
      - limitedRenderSupport : lrs     (bool)          [query]
          This flag when used with query will return whether limited rendering is supported on the current graphics system. Please
          see the graphics card qualification charts for the current definition of limited support.
    
      - lowQualityLighting : lql       (bool)          [create,query]
          Disable lighting evaluation per pixel (fragment). Note: The values passed via the command will override settings stored
          in the hardware render globals node.
    
      - noRenderView : nrv             (bool)          [create,query]
          When turned on, the render view is not updated after image computation
    
      - notWriteToFile : nwf           (bool)          [create,query]
          This flag is set to true if the user does not want to write the image to a file. It is set to false, otherwise. The
      ...
      - renderHardwareName : rhw       (bool)          [query]
          This flag will create a graphics context and return the name of the graphics hardware being used. The graphics hardware
          is determined by creating an off screen buffer and querying the GL_RENDERER string from OpenGL. If the off screen buffer
          cannot be created an empty string is returned.
    
      - renderRegion : reg             (int, int, int, int) [create,query]
          The renderRegion flag pass the render region to the hwRender command. The parameters are 4 integers, indicating left
...
def renderLayerPostProcess(*args, **kwargs):
    ...
    Flags:
      ...
      - sceneName : sn                 (unicode)       [create,query]
          Specifies the scene name for interactive batch rendering.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def renderGlobalsNode(*args, **kwargs):
    ...
    Flags:
      ...
      - renderQuality : rq             (unicode)       [create]
          Set the quality to be the renderQuality node with the given name.Flag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def viewLookAt(*args, **kwargs):
    ...
    Flags:
      - position : pos                 (float, float, float) [create]
          Position in world space to make the camera look at.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def surfaceShaderList(*args, **kwargs):
    ...
    Flags:
      ...
      - remove : rm                    (PyNode)        [create]
          remove object(s) to shader group list.Flag can appear in Create mode of commandFlag can have multiple arguments, passed
          either as a tuple or a list.
...
def editRenderLayerGlobals(*args, **kwargs):
    """
    Edit the parameter values common to all render layers. Some of these paremeters, eg. baseId and mergeType, are stored as
    preferences and some, eg. currentRenderLayer, are stored in the file.
    ...
    Flags:
      ...
      - currentRenderLayer : crl       (PyNode)        [create,query]
          ...
          the render layer. This flag may also be used in conjunction with "useCurrent" to automatically add new DAG objects to
          the active layer. The "isCurrentRenderLayerChanging" condition can be used to determine when the current layer is being
          changed and adjustments are being applied to the scene.
      ...
      - mergeType : mt                 (int)           [create,query]
          Set file import merge type. Valid values are 0, none, 1, by number, and 2, by name.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def exclusiveLightCheckBox(*args, **kwargs):
    ...
    Flags:
      ...
      - light : lt                     (PyNode)        [create]
          The light that is to be made exclusive/non-exclusive.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def convertTessellation(*args, **kwargs):
    ...
    there will be in the tessellation. If the "-allCameras" flags is specified, the renderable camera closest to the surface
    ...
    Flags:
      - allCameras : acm               (bool)          [create]
          Specifies that all renderable cameras should be used in calculating the screen based tessellation.Flag can appear in
          Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def projectionManip(*args, **kwargs):
    ...
    Flags:
      ...
      - switchType : st                (bool)          [create]
          ...
          projection types.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or
          a list.
...
def psdExport(*args, **kwargs):
    """
    Writes the Photoshop file layer set into different formats. The output file depth (bit per channel ) can be different
    from that of the input. If the input is 16 bpc and output is 8 bpc, there will be data loss. In query mode, return type
    ...
    Flags:
      - alphaChannelIdx : aci          (int)           [create,query]
          ...
          This is useful to write out specific alpha channels available as "Additional Alpha Channels" of Photoshop.
      ...
      - emptyLayerSet : els            (bool)          [create,query]
          ...
          set names should be specified.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either
          as a tuple or a list.
...
def pointLight(*args, **kwargs):
    ...
    Flags:
      ...
      - useRayTraceShadows : rs        (bool)          [create,query,edit]
          ray trace shadowsFlag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or
          a list.
...
def psdEditTextureFile(*args, **kwargs):
    ...
    Flags:
      ...
      - deleteChannel : deleteChannel  (unicode)       [create]
          (M) Deletes the channels (layer sets) from a PSD file. This is a multiuse flag.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
      ...
      - uvSnapPostionTop : uvt         (bool)          [create]
          Specifies the position of UV snapshot image layer in the PSD file. "True" positions this layer at the top and "False"
          positions the layer at the bottom next to the background layer in the PSD file
...
def directionalLight(*args, **kwargs):
    ...
    Flags:
      ...
      - softShadow : ss                (bool)          [create,query,edit]
          soft shadowFlag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
...
def getRenderTasks(*args, **kwargs):
    ...
    camera, render layer, and resolution, or other, renderer-specific context. Because of image source overrides, the render
    task context depends on the path through the render dependency graph, with the most upstream override for a context item
    applied. As there can be multiple paths through a render dependency graph to a render dependency, there can be multiple
    render tasks for a given render dependency.
    ...
    Flags:
      ...
      - renderLayer : rl               (unicode)       [create]
          Render layer to use in the render context for the image source render task.Flag can appear in Create mode of commandFlag
          can have multiple arguments, passed either as a tuple or a list.
...
def viewSet(*args, **kwargs):
    ...
    top, side, or front; the view is "fit" based on the list of selected objects (if there are any) or on all the objects if
    nothing is selected. If a camera is not specified, the camera in the active view will be used. If no flag is specified,
    the camera is set to the home position. In query mode, return type is based on queried flag.
    ...
    Flags:
      ...
      - fitFactor : ff                 (float)         [create]
          Specifies how much of the view should be filled with the "fitted" itemsFlag can appear in Create mode of commandFlag can
      ...
      - home : h                       (bool)          [create]
          Executes the camera's home attribute command. Before the string is executed, all occurances of "%camera" will be
          replaced by the camera's name. Use the camera command to set a camera's home command.
...
def frameBufferName(*args, **kwargs):
    ...
    Flags:
      - autoTruncate : a               (bool)          [create]
          ...
          by the destination file format, if applicable.Flag can appear in Create mode of commandFlag can have multiple arguments,
          passed either as a tuple or a list.
...
def sampleImage(*args, **kwargs):
    ...
    Flags:
      ...
      - resolution : r                 (<type 'int'>, PyNode) [create]
          ...
          resolution.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
...
def listCameras(*args, **kwargs):
    ...
    Flags:
      - orthographic : o               (bool)          [create]
          Display all orthographic cameras.Flag can appear in Create mode of commandFlag can have multiple arguments, passed
          either as a tuple or a list.
...
def doBlur(*args, **kwargs):
    """
    The doBlur command will invoke the blur2d, which is a Maya stand-alone application to do 2.5 motion blur given the color
    image and the motion vector file. For a given input colorFile name, e.g. "xxx.iff", the output blurred image will be
    "xxx_blur.iff" in the same directory as the input colorFile. There is currently no control over the name of the output
    blurred image.
    ...
    Flags:
      ...
      - vectorFile : v                 (unicode)       [create]
          Name of the input motion vector file.Flag can appear in Create mode of commandFlag can have multiple arguments, passed
          either as a tuple or a list.
...
def nodeIconButton(*args, **kwargs):
    ...
    Flags:
      - align : al                     (unicode)       [create,query,edit]
          The label alignment. Alignment values are "left", "right", and "center". By default, the label is aligned "center".
      ...
      - command : c                    (script)        [create,query,edit]
          ...
          drag and drop.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
      ...
      - font : fn                      (unicode)       [create,query,edit]
          ...
          "smallPlainLabelFont", "obliqueLabelFont", "smallObliqueLabelFont", "fixedWidthFont" and "smallFixedWidthFont".
      ...
      - image : i                      (unicode)       [create,query,edit]
          ...
          "iconOnly" style is set, the icon will be scaled to the size of the control.
      ...
      - style : st                     (unicode)       [create,query,edit]
          The draw style of the control. Valid styles are "iconOnly", "textOnly", "iconAndTextHorizontal", "iconAndTextVertical",
          and "iconAndTextCentered". (Note: "iconAndTextCentered" is only available on Windows). If the "iconOnly" style is set,
      ...
      - version : ver                  (unicode)       [create,query,edit]
          ...
          number (e.g. "2013", "2014"). Currently only accepts major version numbers (e.g. 2013.5 should be given as "2013").
...
def layeredShaderPort(*args, **kwargs):
    ...
    Flags:
      ...
      - selectedTransparencyControl : stc (unicode)       [create]
          Specifies the name of the UI-control that represents the currently selected layer's transparency.Flag can appear in
          Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def defaultLightListCheckBox(*args, **kwargs):
    ...
    Flags:
      ...
      - shadingGroup : sg              (PyNode)        [create,edit]
          The shading group that is to be connected/disconnected from the defaultLightList.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def imagePlane(*args, **kwargs):
    ...
    It also supports creating and edit. The object passed to the command may either be an imagePlane node, or a camera, in
    which case the command uses the image plane attached to the camera (if any). If no object is passed in, the current
    selection is used. Currently, most movie related queries work only on 64 bit Windows systems.
    
    Flags:
      - camera : c                     (unicode)       [create,query,edit]
          When creating, it will try to attach the created the image plane to the specified camera. If the given camera is
          invalid, creating will fail. When querying, it will query which camera current image plane is attaching to. If it has no
          camera attaches to(like free image plane), it will return empty string. When edit, it will make the image plane attach
          to the new specified camera. If the camera given is invalid, it will do nothing. When the image plane is attached to a
          camera, the image plane's transform node will be set identity. The detach command will not restore the original position
          of the image plane. but the undo command will restore the original position of the image plane.
      ...
      - detach : d                     (bool)          [edit]
          ...
          camera it attaches to and make it a free image plane.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def psdChannelOutliner(*args, **kwargs):
    ...
    Flags:
      - addChild : ach                 (unicode, unicode) [edit]
          This flag should be used along with the "-psdParent/ppa" flag. A string item gets added as a child to the parent
          specifed with "-psdParent/ppa" flag. The next string assigns an associated image name.
      ...
      - numberOfItems : ni             (bool)          [query]
          Total number of items in the control.Flag can appear in Create mode of commandFlag can have multiple arguments, passed
          either as a tuple or a list.
...
def layeredTexturePort(*args, **kwargs):
    ...
    Flags:
      ...
      - selectedIsVisibleControl : svc (unicode)       [create]
          Specifies the name of the UI-control that represents the currently selected layer's visibility.Flag can appear in Create
          mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def lightlink(*args, **kwargs):
    ...
    Flags:
      ...
      - sets : set                     (bool)          [create]
          When querying, specifies whether the result should include sets linked to the queried light/object. Default is true.Flag
...
def shadingConnection(*args, **kwargs):
    ...
    Flags:
      - connectionState : cs           (bool)          [create,query,edit]
          Specifies the state of the connection.On/True/1 means the connection is still active.Off/False/0 means the connection is
          inactive.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def glRenderEditor(*args, **kwargs):
    ...
    Flags:
      ...
      - lookThru : lt                  (unicode)       [create,query,edit]
          Specify which camera the glRender view should be using.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def editRenderLayerAdjustment(*args, **kwargs):
    ...
    Flags:
      ...
      - nodeLog : nlg                  (bool)          [query]
          Output all adjustments for the specified layer sorted by node name.Flag can appear in Create mode of commandFlag can
          have multiple arguments, passed either as a tuple or a list.
...
def assignViewportFactories(*args, **kwargs):
    ...
    Flags:
      ...
      - nodeType : nt                  (unicode)       [create,query,edit]
          The node type.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
...
def setRenderPassType(*args, **kwargs):
    ...
    required become hidden, and new attributes are unhidden and/or created as needed. This allows passIDs to be changed back
    and forth without losing attribute data. It also allows common attributes to be transported from one render pass type to
    another.
    ...
    Flags:
      ...
      - numChannels : n                (int)           [create]
          ...
          implementation supporting the requested number of channels.Flag can appear in Create mode of commandFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def psdTextureFile(*args, **kwargs):
    ...
    Flags:
      ...
      - channels : chs                 (unicode, int, bool) [create]
          ...
          third argument is a boolean, if "true" a layer is created inside the layer set , "false" creates an empty layer set
      ...
      - uvSnapPostionTop : uvt         (bool)          [create]
          Specifies the position of UV snapshot image layer in the PSD file. "True" positions this layer at the top and "False"
          positions the layer at the bottom next to the background layer in the PSD fileFlag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def makebot(*args, **kwargs):
    ...
    Flags:
      ...
      - verbose : v                    (bool)          [create]
          Makebot will provide feedback if this flag is specifiedFlag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def orbit(*args, **kwargs):
    ...
    (<function orbit at 0x14930d9b0>, <function addCmdDocsCallback at 0x1490edb18>, ('orbit', ''), {})
    ...
    Flags:
      ...
      - pivotPoint : pp                (float, float, float) [create]
          Used as the pivot point in the world space.Flag can appear in Create mode of commandFlag can have multiple arguments,
          passed either as a tuple or a list.
...
def track(*args, **kwargs):
    ...
    (<function track at 0x1493340c8>, <function addCmdDocsCallback at 0x1490edb18>, ('track', ''), {})
    
    Flags:
      - down : d                       (float)         [create]
          Set the amount of down translation in unit distance.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def shadingNetworkCompare(*args, **kwargs):
    ...
    Flags:
      ...
      - delete : delete                (bool)          [create]
          Deletes the specified comparison from memory.Flag can appear in Create mode of commandFlag can have multiple arguments,
          passed either as a tuple or a list.
...
def glRender(*args, **kwargs):
    ...
    Flags:
      ...
      - writeDepthMap : wdm            (bool)          [query,edit]
          Enable/disable writing of zdepth to image files.Flag can appear in Create mode of commandFlag can have multiple
          arguments, passed either as a tuple or a list.
...
def renderThumbnailUpdate(*args, **kwargs):
    ...
    thumbnails everywhere will not update to reflect changes to the object until this command is used to toggle to true. In
...
def lightList(*args, **kwargs):
    ...
    Flags:
      ...
      - remove : rm                    (PyNode)        [create]
          remove object(s) to light list.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either
          as a tuple or a list.
...
def convertIffToPsd(*args, **kwargs):
    ...
    Flags:
      ...
      - psdFileName : pfn              (unicode)       [create,query]
          Output file nameFlag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
...
def uvLink(*args, **kwargs):
    ...
    present, the query flag is assumed to be specified. The term "texture" in the context of UV linking is a bit of an
    ...
    Flags:
      ...
      - isValid : iv                   (bool)          [create]
          ...
          used in conjunction with either the -texture flag or the -uvSet flag, but not both at the same time.Flag can appear in
          Create mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def rampColorPort(*args, **kwargs):
    ...
    Flags:
      ...
      - selectedPositionControl : sp   (unicode)       [create,edit]
          ...
          selected entry if edited. The type of control must be an attrFieldSliderGrp.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def cameraView(*args, **kwargs):
    ...
    Flags:
      ...
      - bookmarkType : typ             (int)           [create]
          Specify the bookmark type, which can be: 0. 3D bookmark; 1. 2D Pan/Zoom bookmark.Flag can appear in Create mode of
          commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def renderSettings(*args, **kwargs):
    ...
    Flags:
      ...
      - customTokenString : cts        (unicode)       [create]
          ...
          pairs. For example, if the file name string is "myFile_myToken_myOtherToken_v" then the argument to this flag string
      ...
      - leaveUnmatchedTokens : lut     (bool)          [create]
          Do not remove unmatched tokens from the name string. Use with firstImageName or lastImageName.Flag can appear in Create
          mode of commandFlag can have multiple arguments, passed either as a tuple or a list.
...
def createLayeredPsdFile(*args, **kwargs):
    ...
    Flags:
      - imageFileName : ifn            (unicode, unicode, unicode) [create]
          ...
          "Normal", "Dissolve", "Dark", "Multiply", "Color Burn", "Linear Burn", "Lighten", "Screen", "Color Dodge", "Linear
          Dogde", "Overlay", "Soft Light", "Hard Light", "Dissolve", "Vivid Light", "Linear Light", "Pin Light", "Hard Mix",
          "Difference", "Exclusion", "Hue", "Saturation", "Color", "Luminosity"
    
      - psdFileName : psf              (unicode)       [create]
          PSD file name.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either as a tuple or a
          list.
...
def roll(*args, **kwargs):
    ...
    Flags:
      ...
      - degree : d                     (float)         [create]
          Set the amount of the rotation angle.Flag can appear in Create mode of commandFlag can have multiple arguments, passed
          either as a tuple or a list.
...
def surfaceSampler(*args, **kwargs):
    ...
    Flags:
      ...
      - flipV : fv                     (bool)          [create]
          Flip the V coordinate of the generated image.Flag can appear in Create mode of commandFlag can have multiple arguments,
          passed either as a tuple or a list.
      ...
      - maxSearchDistance : msd        (float)         [create]
          ...
          indicates no limit. When generated maps include artifacts from the "other side" of an object, try setting this value to
          a distance approximately equal to the radius of the object. If this flag is included, it must be included once for every
          target.
...
def prepareRender(*args, **kwargs):
    ...
    a rendering traversal can activate an appropriate representation, for each assembly node in the scene. When rendering is
    done, this command can correspondingly restore the representation that was active before rendering on each assembly.
    ...
    invokes the callbacks of the default traversal set. The prepareRender scripting capability allows for the development of
    multiple rendering preparation scripts, including from plugins, to provide extensibility rather than being constrained
    to a single implementation. A special traversal set is the "null" traversal set. It is the initial default traversal
    set, and cannot be deregistered. It performs no work, and does not save and restore the assembly node active
    ...
    views or batch render code. Most uses of this command will simply use the edit mode to register render traversals into a
    render traversal set, or the query mode to query the names of the render traversals in a render traversal set. In query
    ...
    Flags:
      ...
      - deregister : d                 (unicode)       [edit]
          ...
          traversal set will be the "null" traversal set.
      ...
      - postRenderFrame : pof          (script)        [query,edit]
          Set or query the postRenderFrame render traversal for a given traversal set. This traversal is run after the render of a
          single frame, with a render layer. The traversal set will be the default traversal set, unless the -traversalSet flag is
      ...
      - postRenderLayer : pol          (script)        [query,edit]
          ...
          is rendered, within a render. The traversal set will be the default traversal set, unless the -traversalSet flag is used
      ...
      - preRenderFrame : prf           (script)        [query,edit]
          Set or query the preRenderFrame render traversal for a given traversal set. This traversal is run before the render of a
          single frame, with a render layer. The traversal set will be the default traversal set, unless the -traversalSet flag is
          used to specify an explicit traversal set.
      ...
      - saveAssemblyConfig : sac       (bool)          [query,edit]
          ...
          given traversal set. The traversal set will be the default, unless the -traversalSet flag is used to specify an explicit
          traversal set.
      ...
      - traversalSetInit : tsi         (script)        [query,edit]
          ...
          whenever the specified traversal set becomes the default. traversal set.Flag can appear in Create mode of commandFlag
          can have multiple arguments, passed either as a tuple or a list.
...
def viewHeadOn(*args, **kwargs):
    """
    The viewHeadOn command positions the specified camera so it is looking "down" the normal of the live object, and fitted
    to the live object. If the live object is a surface, an arbitrary normal is chosen.
...
def renderManip(*args, **kwargs):
    ...
    Flags:
      ...
      - light : lt                     (bool, bool, bool) [query,edit]
          ...
          center of interest, and pivot.Flag can appear in Create mode of commandFlag can have multiple arguments, passed either
          as a tuple or a list.
